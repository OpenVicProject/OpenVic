...
func _ready() -> void:
	...
	if not _map_mesh_instance.mesh is MapMesh:
		push_error(
			"Invalid map mesh class: ",
			_map_mesh_instance.mesh.get_class(),
			"(expected MapMesh)",
		)
		return
...
func look_at_map_position(pos: Vector2) -> void:
	var viewport_centre: Vector2 = Vector2(
		0.5,
		0.5,
	) * _viewport_dims / get_tree().root.content_scale_factor
...
func set_hovered_province_number(province_number: int) -> void:
	if _map_shader_material:
		_map_shader_material.set_shader_parameter(
			GameLoader.ShaderManager.param_hover_index,
			province_number,
		)
...
func _on_province_selected(province_number: int) -> void:
	if _map_shader_material:
		_map_shader_material.set_shader_parameter(
			GameLoader.ShaderManager.param_selected_index,
			province_number,
		)
	...
	if event is InputEventMouseMotion:
		...
		if _mouse_over_viewport:
			if _map_mesh.is_valid_uv_coord(_mouse_pos_map):
				...
				if port_province_number != 0:
					...
					selectionMarkers.toggle_id_selected(
						5000 + port_province_number,
						_map_to_world_coords(port_pos),
					)
				else:
		...
		if _mouse_over_viewport:
			if _map_mesh.is_valid_uv_coord(_mouse_pos_map):
				...
				if port_province_number != 0:
					var port_pos: Vector2 = MapItemSingleton.get_port_position_by_province_number(port_province_number)
					validMoveMarkers.add_move_marker(
						_map_to_world_coords(port_pos),
						randi_range(0, 1),
					)
				else:
				...
				# TODO - open diplomacy screen on province owner or viewed country if province has
				# no owner
...
func _clamp_over_map() -> void:
	_camera.position.x = _map_mesh_corner.x + fposmod(
		_camera.position.x - _map_mesh_corner.x,
		_map_mesh_dims.x,
	)
	_camera.position.z = clampf(
		_camera.position.z,
		_map_mesh_corner.y,
		_map_mesh_corner.y + _map_mesh_dims.y,
	)
...
func _zoom_process(delta: float) -> void:
	...
	if _map_shader_material:
		_map_shader_material.set_shader_parameter(
			GameLoader.ShaderManager.param_parchment_mix,
			float(parchment_mapmode),
		)
