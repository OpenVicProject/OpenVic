	...
	for anim_Chunk: BoneAnimationChunk in boneAnimationChunks:
		for submotion: SkeletalSubMotion in anim_Chunk.SkeletalSubMotions:
			...
			var skeleton_path: String = (
				SKELETON_PATH % FileAccessUtils.replace_chars(submotion.nodeName)
			)
...
class MetadataChunk:
	...
	func debugPrint() -> void:
		print(
			(
				"FileName: %s, sourceApp: %s, exportDate: %s, ExporterV:%d.%d"
				% [origFileName, sourceApp, exportDate, exporterMajorVersion, exporterMinorVersion]
			)
		)
		print("MotionName: %s, fps: %d, MaxError: %s" % [motionName, fps, fMaxAcceptableError])
...
static func readSkeletalSubMotion(file: FileAccess, use_quat16: bool) -> SkeletalSubMotion:
	...
	var submotion: SkeletalSubMotion = SkeletalSubMotion.new(
		a, b, c, d, e, f, g, h, p, j, k, l, m, n  # quats  # vec3  # ints
	)
...
class SkeletalSubMotion:
	...
	func debugPrint() -> void:
		print(
			(
				"Node: %s, #PosKeys %d, #RotKeys %d, #ScaleKeys %d, #ScaleRotKeys %d"
				% [nodeName, numPosKeys, numRotKeys, numScaleKeys, numScaleRotKeys]
			)
		)
		print(
			(
				"\tposeScaleRot %s,\tbindPoseScaleRot %s,\tposeScale %s,\tbindPoseScale %s"
				% [poseScaleRot, bindPoseScaleRot, poseScale, bindPoseScale]
			)
		)
