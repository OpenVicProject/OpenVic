...
const SCROLLING_MATERIAL_FACTORS: Dictionary = {"TexAnim": 2.5, "Smoke": 0.3}  # Tank tracks  # Buildings, factories, steam ships, sieges


static func setup_flag_shader() -> void:
	flag_shader.set_shader_parameter(&"flag_dims", GameSingleton.get_flag_dims())
	flag_shader.set_shader_parameter(
		&"texture_flag_sheet_diffuse", GameSingleton.get_flag_sheet_texture()
	)
...
static func _load_xac_model(source_file: String, is_unit: bool) -> Node3D:
	...
	node.name = metaDataChunk.origFileName.replace("\\", "/").split("/", false)[-1].get_slice(
		".", 0
	)
	...
	for chunk: MeshChunk in mesh_chunks:
		...
		if influenceRangeInd.is_empty() or skinningChunks.is_empty() or not applyVertexWeights:
			if mesh_chunks.size() != 1 and mesh_chunk_name in INVALID_IF_NOT_ONLY_MESH:
				push_warning(
					'Skipping unused mesh "',
					mesh_chunk_name,
					'" in model "',
					node.name,
					'" because it was not the only mesh chunk in its file'
				)
				break
		...
		if not verts.is_empty():
			...
			for submesh: SubMesh in chunk.SubMeshes:
				...
				for i: int in submesh.relativeIndices.size():
					...
					if (
						not influenceRangeInd.is_empty()
						and not skinningChunks.is_empty()
						and applyVertexWeights
					):
						...
						var influenceRange: InfluenceRange = (
							skin_chunk.influenceRange[vert_inf_range_ind]
						)
						var boneWeights: Array[InfluenceData] = (
							skinningChunks[skinning_chunk_ind]
							. influenceData
							. slice(
				...
				if materials[submesh.materialId].spec_index != -1:
					meshInstance.set_instance_shader_parameter(
						&"tex_index_specular", materials[submesh.materialId].spec_index
					)

				if materials[submesh.materialId].diffuse_index != -1:
					meshInstance.set_instance_shader_parameter(
						&"tex_index_diffuse", materials[submesh.materialId].diffuse_index
					)

				if materials[submesh.materialId].scroll_index != -1:
					meshInstance.set_instance_shader_parameter(
						&"scroll_tex_index_diffuse", materials[submesh.materialId].scroll_index
					)
...
class MaterialDefinition:
	...
	func _init(
		mat: Material, diffuse_ind: int = -1, spec_ind: int = -1, scroll_ind: int = -1
	) -> void:
		self.mat = mat
...
static func make_materials(
	materialDefinitionChunks: Array[MaterialDefinitionChunk]
) -> Array[MaterialDefinition]:
	...
	for matdef: MaterialDefinitionChunk in materialDefinitionChunks:
			...
			match layer.mapType:
				2:  # diffuse
					if not diffuse_name:
						...
						push_error(
							"Multiple diffuse layers in material: ",
							diffuse_name,
							" and ",
							layer.texture
						)
				...
				3:  # specular
					if not specular_name:
						...
						push_error(
							"Multiple specular layers in material: ",
							specular_name,
							" and ",
							layer.texture
						)
				...
				5:  # normal
					if not normal_name:
						...
						push_error(
							"Multiple normal layers in material: ",
							normal_name,
							" and ",
							layer.texture
						)
		...
		if diffuse_name and specular_name:
			...
			if textures_index_spec < 0:
				var unit_colours_mask_texture: ImageTexture = AssetManager.get_texture(
					TEXTURES_PATH % specular_name
				)
				if unit_colours_mask_texture:
					...
					if len(added_unit_textures_spec) >= MAX_UNIT_TEXTURES:
						push_error(
							"Colour masks have exceeded max number of textures supported by unit shader!"
						)
			...
			if textures_index_diffuse < 0:
				var diffuse_texture: ImageTexture = AssetManager.get_texture(
					TEXTURES_PATH % diffuse_name
				)
				if diffuse_texture:
					...
					if len(added_unit_textures_diffuse) >= MAX_UNIT_TEXTURES:
						push_error(
							"Diffuse textures have exceeded max number supported by unit shader!"
						)

					const param_texture_diffuse: StringName = &"texture_diffuse"

					var diffuse_textures: Array = unit_shader.get_shader_parameter(
			...
			materials.push_back(
				MaterialDefinition.new(unit_shader, textures_index_diffuse, textures_index_spec)
			)
		...
		# Flag (diffuse is unionjacksquare which is ignored)
			...
			var flag_normal_texture: ImageTexture = AssetManager.get_texture(
				TEXTURES_PATH % normal_name
			)
		...
		# Scrolling texture
			...
			var scroll_textures_index_diffuse: int = added_scrolling_textures_diffuse.find(
				diffuse_name
			)
			if scroll_textures_index_diffuse < 0:
				...
				if diffuse_texture:
					...
					if len(added_scrolling_textures_diffuse) >= MAX_SCROLLING_TEXTURES:
						push_error(
							"Diffuse textures have exceeded max number supported by scrolling shader!"
						)

					const param_scroll_texture_diffuse: StringName = &"scroll_texture_diffuse"

					var scroll_diffuse_textures: Array = scrolling_shader.get_shader_parameter(
						param_scroll_texture_diffuse
					)
					scroll_diffuse_textures.push_back(diffuse_texture)
					scroll_textures_index_diffuse = len(scroll_diffuse_textures) - 1
					scrolling_shader.set_shader_parameter(
						param_scroll_texture_diffuse, scroll_diffuse_textures
					)

					const param_scroll_factor: StringName = &"scroll_factor"

					var scroll_factors: Array = scrolling_shader.get_shader_parameter(
						param_scroll_factor
					)
			...
			materials.push_back(
				MaterialDefinition.new(scrolling_shader, -1, -1, scroll_textures_index_diffuse)
			)
		...
		# Standard material
			...
			if normal_name:
				var normal_texture: ImageTexture = AssetManager.get_texture(
					TEXTURES_PATH % normal_name
				)
				if normal_texture:
...
static func build_armature(hierarchy_chunk: NodeHierarchyChunk) -> Skeleton3D:
	...
	for node: NodeData in hierarchy_chunk.nodes:
		...
		skeleton.set_bone_rest(
			cur_id, Transform3D(Basis(node.rotation).scaled(node.scale), node.position)
		)
...
static func readMetaDataChunk(file: FileAccess) -> MetadataChunk:
	return MetadataChunk.new(
		...
		file.get_8(),
		file.get_8(),
		Vector2i(file.get_8(), file.get_8()),
		file.get_float(),
...
class MetadataChunk:
	...
	func debugPrint() -> void:
		print(
			(
				"Ver: %d.%d, exported: %s, fileName: %s, sourceApp: %s"
				% [exporterMajorVersion, exporterMinorVersion, exportDate, origFileName, sourceApp]
			)
		)
		print(
			(
				"Actor: %s, retargetRootOffset: %d, repositionMask: %d, repositionNode: %d"
...
static func readNodeData(file: FileAccess) -> NodeData:
	return NodeData.new(
		FileAccessUtils.read_quat(file),
		FileAccessUtils.read_quat(file),
		FileAccessUtils.read_pos(file),
		FileAccessUtils.read_vec3(file),
		...
		FileAccessUtils.read_int32(file),
		FileAccessUtils.read_int32(file),
		FileAccessUtils.read_mat4x4(file),
		file.get_float(),
		FileAccessUtils.read_xac_str(file)
	)
...
class NodeData:
	...
	func debugPrint() -> void:
		print(
			(
				"\tparentNodeId: %d,\t numChildNodes: %d,\t Node Name: %s"
				% [parentNodeId, numChildNodes, name]
			)
		)
		print(
			(
				"\tunused %s,%s,%s, -1: %s, -1: %s"
				% [unused[0], unused[1], unused[2], unknown, unknown2]
			)
		)
...
static func readMaterialTotalsChunk(file: FileAccess) -> MaterialTotalsChunk:
	return MaterialTotalsChunk.new(
		FileAccessUtils.read_int32(file),
		FileAccessUtils.read_int32(file),
		FileAccessUtils.read_int32(file)
	)
...
class MaterialTotalsChunk:
	...
	func debugPrint() -> void:
		print(
			(
				"totalMaterials: %d, standardMaterials: %d, fxMaterials: %d"
...
static func readLayer(file: FileAccess, isV1: bool) -> Layer:
	...
	var layer: Layer = Layer.new(
		...
		file.get_float(),
		file.get_float(),
		FileAccessUtils.read_int16(file),
		file.get_8(),
		file.get_8(),
		FileAccessUtils.read_xac_str(file),
		unknown
...
class Layer:
	...
	func debugPrint() -> void:
		print(
			(
				"\tLayer MatId:%d,\t UVOffset:%d,%d,\t UVTiling %d,%d mapType: %d,\t Texture Name: %s"
...
static func readMaterialDefinitionChunk(file: FileAccess, isV1: bool) -> MaterialDefinitionChunk:
	var chunk: MaterialDefinitionChunk = MaterialDefinitionChunk.new(
		FileAccessUtils.read_vec4(file),
		FileAccessUtils.read_vec4(file),
		FileAccessUtils.read_vec4(file),
		FileAccessUtils.read_vec4(file),
		file.get_float(),
...
class MaterialDefinitionChunk:
	...
	func debugPrint() -> void:
		...
		print(
			(
				"\tshine:%s\tshineStrength:%s\t,opacity:%s,\tior:%s,\tunused:%s"
				% [shine, shineStrength, opacity, ior, unused]
			)
		)
		for layer: Layer in Layers:
			layer.debugPrint()
...
class VerticesAttribute:
	...
	func _init(
		type: int, attribSize: int, bKeepOriginals: bool, bIsScaleFactor: bool, pad: Vector2i
	) -> void:
		self.type = type
	...
	func debugPrint() -> void:
		...
		print(
			(
				"\tattribSize:%d (bytes),\t keepOriginals:%s,\t isScaleFactor:%s,\t VertAttrib: type:%s"
...
class SubMesh:
	...
	func debugPrint() -> void:
		print(
			(
				"\tSubMesh:\t numIndices:%d,\t numVerts:%d,\t matId:%d,\t numBones:%d"
...
class MeshChunk:
	...
	func debugPrint() -> void:
		print(
			(
				"Mesh: nodeId:%d, numVerts:%d, numSubMeshes:%d, numVertAttribs:%d, isCollisionMesh:%s"
...
static func readInfluenceData(file: FileAccess) -> InfluenceData:
	return InfluenceData.new(
		file.get_float(), FileAccessUtils.read_int16(file), Vector2i(file.get_8(), file.get_8())
	)
...
class InfluenceRange:
	...
	func debugPrint() -> void:
		print(
			(
				"\tInfluenceRange:\t firstIndex: %d,\t numInfluences: %d"
				% [firstInfluenceIndex, numInfluences]
			)
		)


static func readSkinningChunk(
	file: FileAccess, meshChunks: Array[MeshChunk], isV2: bool
) -> SkinningChunk:
	var skinning: SkinningChunk = null
	skinning = SkinningChunk.new(
		FileAccessUtils.read_int32(file),
		-1 if isV2 else FileAccessUtils.read_int32(file),
		FileAccessUtils.read_int32(file),
		file.get_8(),
		Vector3i(file.get_8(), file.get_8(), file.get_8())
	)
	...
	for chunk: MeshChunk in meshChunks:
		if (
			chunk.nodeId == skinning.nodeId
			and chunk.bIsCollisionMesh == skinning.bIsForCollisionMesh
		):
...
class SkinningChunk:
	...
	func debugPrint() -> void:
		print(
			(
				"Skinning: nodeId:%d, numInfluencedBones: %d, numInfluences: %d, CollisionMesh?: %d"
...
class ChunkType6:
	...
	func _init(
		unknown: PackedInt32Array, unknown_floats: PackedFloat32Array, maybe_node_id: int
	) -> void:
		self.unknown = unknown
	...
	func debugPrint() -> void:
		print(
			(
				"\tUnknown: %s\n\tFloats: %s\n\tPerhaps NodeId? %s"
				% [self.unknown, self.unknown_floats, self.maybe_node_id]
			)
		)
...
static func readNodeChunk(file: FileAccess) -> NodeChunk:
	return NodeChunk.new(
		...
		FileAccessUtils.read_int32(file),
		FileAccessUtils.read_int32(file),  #-1-1
