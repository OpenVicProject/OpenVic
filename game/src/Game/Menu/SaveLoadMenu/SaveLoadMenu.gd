...
func _on_save_line_edit_text_submitted(new_text) -> void:
	...
	if SaveManager.has_save(new_text):
		_overwrite_dialog.dialog_text = tr("SAVELOADMENU_OVERWRITE_DIALOG_TEXT").format(
			{"file_name": _submitted_text}
		)
		_overwrite_dialog.title = tr("SAVELOADMENU_OVERWRITE_DIALOG_TITLE").format(
			{"file_name": _submitted_text}
		)
...
func _on_save_node_delete_requested(node: Control) -> void:
	...
	_delete_dialog.dialog_text = tr("SAVELOADMENU_DELETE_DIALOG_TEXT").format(
		{"file_name": _requested_node_to_delete.resource.save_name}
	)
	_delete_dialog.title = tr("SAVELOADMENU_DELETE_DIALOG_TITLE").format(
		{"file_name": _requested_node_to_delete.resource.save_name}
	)
