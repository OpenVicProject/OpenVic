...
@export var _version_label: Button

@export var _commit_label: Button

@export var _checksum_label: Button
...
func _ready() -> void:
	_version_label.text = tr("MAIMENU_LATEST_RELEASE_NAME").format(
		{"release_name": _GIT_INFO_.release_name}
	)
...
func _update_checksum_label_text() -> void:
	...
	_checksum_label.text = tr("MAINMENU_CHECKSUM").format(
		{"short_checksum": _checksum.substr(0, 4)}
	)
...
func _on_game_info_button_pressed() -> void:
	...
	DisplayServer.clipboard_set(
		(
			"%s %s (%s) [Godot %s] - %s %s - %s - %s (API: %s) - %s (%s Threads)"
			% [
				project_name,
				tag_name,
				commit_sha,
