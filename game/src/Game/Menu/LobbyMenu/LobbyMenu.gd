...
func _on_start_button_pressed() -> void:
	...
	if SaveManager.current_save == null and SaveManager.current_session_tag in _id_to_tag:
		session_tag_dialog.dialog_text = tr("GAMELOBBY_SESSIONTAG_DIALOG_TEXT").format(
			{"session_tag": SaveManager.current_session_tag}
		)
		session_tag_dialog.title = tr("GAMELOBBY_SESSIONTAG_DIALOG_TITLE").format(
			{"session_tag": SaveManager.current_session_tag}
		)
...
func _on_save_node_delete_requested(node: Control) -> void:
	...
	delete_dialog.dialog_text = tr("GAMELOBBY_DELETE_DIALOG_TEXT").format(
		{"file_name": _requested_node_to_delete.resource.save_name}
	)
	delete_dialog.title = tr("GAMELOBBY_DELETE_DIALOG_TITLE").format(
		{"file_name": _requested_node_to_delete.resource.save_name}
	)
