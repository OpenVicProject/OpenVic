...
var last_validated_ip : Array[int] = []
...
func join_game(ip : String, player_name : String) -> bool:
	...
	var ip_segments : Array[int] = validate_ipv4(ip)
	print(ip_segments)
...
func _init() -> void:
	var regex_res := IP_REGEX.compile(IP_PATTERN)
	if regex_res != OK:
...
func validate_ipv4(ip : String) -> Array[int]:
...
func load_saved_ips() -> void:
	...
	var game_name : StringName = file.get_value(MP.USER,MP.HOST_GAME_NAME)
	var game_password : StringName = file.get_value(MP.USER,MP.HOST_GAME_PASSWORD)
