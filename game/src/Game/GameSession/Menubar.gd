...
func _add_mapmode_button() -> void:
	...
	var button: GUIIconButton = get_gui_icon_button_from_nodepath(
		"./menubar/mapmode_%d" % (index + 1)
	)
	if button:
...
func _ready() -> void:
	...
	set_click_mask_from_nodepaths([^"./menubar/minimap_bg", ^"./menubar/menubar_bg"])
	...
	var zoom_out_button: GUIIconButton = get_gui_icon_button_from_nodepath(
		^"./menubar/map_zoom_out"
	)
	if zoom_out_button:
...
func _mapmode_pressed(index: int) -> void:
	...
	print(
		(
			'Mapmode set to "%s" (index: %d, identifier: %s)'
			% [
				tr(GameSingleton.get_mapmode_localisation_key(index)),
				index,
				GameSingleton.get_mapmode_identifier(index)
			]
		)
	)
...
func _on_mapmode_changed(index: int) -> void:
	...
	var new_mapmode_button: GUIIconButton = (
		_mapmode_buttons[index] if 0 <= index and index < _mapmode_buttons.size() else null
	)
...
func _minimap_gui_input(_event: InputEvent) -> void:
	...
	if Input.is_action_pressed(_action_click):
		var pos_clicked: Vector2 = (
			_minimap_icon.get_local_mouse_position() / _minimap_icon.size - Vector2(0.5, 0.5)
		)
