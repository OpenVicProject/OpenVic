...
class BuildingSlot:
	...
	func _init(new_slot_index: int, new_slot_node: Control) -> void:
		...
		if _expand_button:
			_expand_button.pressed.connect(
				func() -> void: MenuSingleton.expand_selected_province_building(_slot_index)
			)
		_expanding_icon = GUINode.get_gui_icon_from_node(
			_slot_node.get_node(^"./underconstruction_icon")
		)
		_expanding_progress_bar = GUINode.get_gui_progress_bar_from_node(
			_slot_node.get_node(^"./building_progress")
		)
	...
	func update_info(info: Dictionary) -> void:
		...
		var expansion_state: int = info.get(
			building_info_expansion_state_key, ExpansionState.CannotExpand
		)
		var expansion_in_progress: bool = (
			expansion_state == ExpansionState.Preparing
			or expansion_state == ExpansionState.Expanding
		)
...
func _ready() -> void:
	...
	var close_button: GUIIconButton = get_gui_icon_button_from_nodepath(
		^"./province_view/close_button"
	)
	if close_button:
		close_button.pressed.connect(_on_close_button_pressed)

	# Header
	_province_name_label = get_gui_label_from_nodepath(
		^"./province_view/province_view_header/province_name"
	)
	_state_name_label = get_gui_label_from_nodepath(
	...
	_slave_status_icon = get_gui_icon_from_nodepath(
		^"./province_view/province_view_header/slave_state_icon"
	)
	if _slave_status_icon:
		_slave_status_icon.set_icon_index(MenuSingleton.get_slave_pop_icon_index())
	_colony_status_button = get_gui_icon_button_from_nodepath(
		^"./province_view/province_view_header/colony_button"
	)
	var admin_icon: GUIIcon = get_gui_icon_from_nodepath(
		^"./province_view/province_view_header/admin_icon"
	)
	if admin_icon:
		admin_icon.set_icon_index(MenuSingleton.get_administrative_pop_icon_index())
	_administrative_percentage_label = get_gui_label_from_nodepath(
		^"./province_view/province_view_header/admin_efficiency"
	)
	_owner_percentage_label = get_gui_label_from_nodepath(
		^"./province_view/province_view_header/owner_presence"
	)
	_province_modifiers_overlapping_elements_box = get_gui_overlapping_elements_box_from_nodepath(
		^"./province_view/province_view_header/province_modifiers"
	)
	if (
		_province_modifiers_overlapping_elements_box
		and (
			_province_modifiers_overlapping_elements_box.set_gui_child_element_name(
				"province_interface", "prov_state_modifier"
			)
			!= OK
		)
	):
		_province_modifiers_overlapping_elements_box = null  # hide province modifiers box since we can't do anything with it
	_terrain_type_icon = get_gui_icon_from_nodepath(
		^"./province_view/province_view_header/prov_terrain"
	)
	_life_rating_bar = get_gui_progress_bar_from_nodepath(
		^"./province_view/province_view_header/liferating"
	)
	_controller_flag = get_gui_masked_flag_from_nodepath(
		^"./province_view/province_view_header/controller_flag"
	)

	# Statistics
	_rgo_icon = get_gui_icon_from_nodepath(^"./province_view/province_statistics/goods_type")
	_rgo_produced_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/produced"
	)
	_rgo_income_label = get_gui_label_from_nodepath(^"./province_view/province_statistics/income")
	_rgo_employment_percentage_icon = get_gui_icon_from_nodepath(
		^"./province_view/province_statistics/employment_ratio"
	)
	_rgo_employment_population_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/rgo_population"
	)
	_rgo_employment_percentage_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/rgo_percent"
	)
	_crime_name_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/crime_name"
	)
	_crime_icon = get_gui_icon_from_nodepath(^"./province_view/province_statistics/crime_icon")
	_crime_fighting_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/crimefight_percent"
	)
	_total_population_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/total_population"
	)
	_migration_label = get_gui_label_from_nodepath(^"./province_view/province_statistics/migration")
	_population_growth_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/growth"
	)
	_pop_types_piechart = get_gui_pie_chart_from_nodepath(
		^"./province_view/province_statistics/workforce_chart"
	)
	_pop_ideologies_piechart = get_gui_pie_chart_from_nodepath(
		^"./province_view/province_statistics/ideology_chart"
	)
	_pop_cultures_piechart = get_gui_pie_chart_from_nodepath(
		^"./province_view/province_statistics/culture_chart"
	)
	var population_menu_button: GUIIconButton = get_gui_icon_button_from_nodepath(
		^"./province_view/province_statistics/open_popscreen"
	)
		...
		population_menu_button.pressed.connect(
			func() -> void:
				...
				Events.NationManagementScreens.open_nation_management_screen(
					NationManagement.Screen.POPULATION
				)
		)
	_supply_limit_label = get_gui_label_from_nodepath(
		^"./province_view/province_statistics/supply_limit_label"
	)
	_cores_overlapping_elements_box = get_gui_overlapping_elements_box_from_nodepath(
		^"./province_view/province_statistics/core_icons"
	)
	if (
		_cores_overlapping_elements_box
		and (
			_cores_overlapping_elements_box.set_gui_child_element_name(
				"province_interface", "province_core"
			)
			!= OK
		)
	):
	...
	if _buildings_panel:
		...
		for current_slot_count: int in target_slot_count:
			var slot := GUINode.generate_gui_element(
				"province_interface", "building", "building_slot_%d" % current_slot_count
			)
			if slot:
				...
				push_error(
					"Failed to generate building slot ",
					current_slot_count,
					" / ",
					target_slot_count
				)
...
func _update_info() -> void:
	...
	if _province_info:
		...
		if _province_name_label:
			_province_name_label.text = GUINode.format_province_name(
				_province_info.get(_province_info_province_key, _missing_suffix)
			)

		if _state_name_label:
			_state_name_label.text = _province_info.get(
				_province_info_state_key, tr(_province_info_state_key + _missing_suffix)
			)
		...
		if _province_modifiers_overlapping_elements_box:
			...
			for i: int in _province_modifiers_overlapping_elements_box.get_child_count():
				var button: GUIIconButton = GUINode.get_gui_icon_button_from_node(
					_province_modifiers_overlapping_elements_box.get_child(i).get_node(
						^"./modifier"
					)
				)
				if button:
		...
		if _terrain_type_icon:
			...
			if terrain_type:
				...
				if (
					_terrain_type_icon.set_gfx_texture_sprite_name(
						_terrain_type_prefix + terrain_type
					)
					!= OK
				):
		...
		if _controller_flag:
			_controller_flag.set_flag_country_name(
				_province_info.get(_province_info_controller_key, "")
			)
		...
		if _rgo_produced_label:
			_rgo_produced_label.text = GUINode.float_to_string_dp(
				_province_info.get(_province_info_rgo_output_quantity_yesterday_key, 0), 3
			)

		if _rgo_income_label:
			_rgo_income_label.text = (
				"%s¤"
				% GUINode.float_to_string_dp(
					_province_info.get(_province_info_rgo_revenue_yesterday_key, 0), 3
				)
			)

		var rgo_employment_tooltip: String = _province_info.get(
			_province_info_rgo_employment_tooltip_key, ""
		)

		if _rgo_employment_percentage_icon:
			_rgo_employment_percentage_icon.set_icon_index(
				int(_province_info.get(_province_info_rgo_employment_percentage_key, 0) / 10) + 1
			)
			_rgo_employment_percentage_icon.set_tooltip_string(rgo_employment_tooltip)

		if _rgo_employment_population_label:
			_rgo_employment_population_label.text = GUINode.int_to_string_suffixed(
				_province_info.get(_province_info_rgo_total_employees_key, 0)
			)
			_rgo_employment_population_label.set_tooltip_string(rgo_employment_tooltip)

		if _rgo_employment_percentage_label:
			_rgo_employment_percentage_label.text = (
				"%d%%" % _province_info.get(_province_info_rgo_employment_percentage_key, 0)
			)
		...
		if _total_population_label:
			_total_population_label.text = GUINode.int_to_string_suffixed(
				_province_info.get(_province_info_total_population_key, 0)
			)
		...
		if _pop_types_piechart:
			_pop_types_piechart.set_slices_array(
				_province_info.get(_province_info_pop_types_key, [])
			)

		if _pop_ideologies_piechart:
			_pop_ideologies_piechart.set_slices_array(
				_province_info.get(_province_info_pop_ideologies_key, [])
			)

		if _pop_cultures_piechart:
			_pop_cultures_piechart.set_slices_array(
				_province_info.get(_province_info_pop_cultures_key, [])
			)
		...
		if _cores_overlapping_elements_box:
			...
			for core_index: int in min(
				cores.size(), _cores_overlapping_elements_box.get_child_count()
			):
				_set_core_flag(core_index, cores[core_index])
			for core_index: int in range(
				cores.size(), _cores_overlapping_elements_box.get_child_count()
			):
		...
		# Buildings
		if _buildings_panel:
			var buildings: Array[Dictionary] = _province_info.get(
				_province_info_buildings_key, [] as Array[Dictionary]
			)
