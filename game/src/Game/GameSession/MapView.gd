		...
		if _mouse_over_viewport:
			if _map_mesh.is_valid_uv_coord(_mouse_pos_map):
				...
				if port_province_index != 0:
					...
					selectionMarkers.add_selection_marker(5000 + port_province_index,_map_to_world_coords(port_pos))
				else:
					var unit_position : Vector2 = MapItemSingleton.get_unit_position_by_province_index(province_index)
					selectionMarkers.add_selection_marker(province_index,_map_to_world_coords(unit_position))
				temp_id += 1
