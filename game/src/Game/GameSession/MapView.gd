...
signal map_view_camera_changed(
	near_left: Vector2, far_left: Vector2, far_right: Vector2, near_right: Vector2
)
...
var _zoom_target: float = _zoom_target_max:
	get:
		return _zoom_target
	set(v):
		_zoom_target = clamp(v, _zoom_target_min, _zoom_target_max)
...
@export
var _zoom_parchment_threshold: float = _zoom_target_min + (_zoom_target_max - _zoom_target_min) / 4
...
func _ready() -> void:
	...
	if not _map_mesh_instance.mesh is MapMesh:
		push_error(
			"Invalid map mesh class: ", _map_mesh_instance.mesh.get_class(), "(expected MapMesh)"
		)
		return
	...
	if not _map_background_instance.mesh is PlaneMesh:
		push_error(
			"Invalid map background mesh class: ",
			_map_background_instance.mesh.get_class(),
			"(expected PlaneMesh)"
		)
		return
	var scaled_dims: Vector3 = (
		_map_background_instance.transform.affine_inverse()
		* Vector3(_map_mesh_dims.x, 0.0, _map_mesh_dims.y)
	)
...
func look_at_map_position(pos: Vector2) -> void:
	var viewport_centre: Vector2 = (
		Vector2(0.5, 0.5) * _viewport_dims / GuiScale.get_current_guiscale()
	)
...
func zoom_in() -> void:
	...
	_zoom_position = (
		(Vector2(0.5, 0.5) - _mouse_pos_viewport * GuiScale.get_current_guiscale() / _viewport_dims)
...
func set_hovered_province_index(hover_index: int) -> void:
	_map_shader_material.set_shader_parameter(
		GameLoader.ShaderManager.param_hover_index, hover_index
	)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		event.is_action(_action_north)
		or event.is_action(_action_south)
		or event.is_action(_action_east)
		or event.is_action(_action_west)
	):
		_cardinal_movement_vector = (
			Input.get_vector(_action_west, _action_east, _action_north, _action_south)
		...
		if _mouse_over_viewport:
			...
			if _map_mesh.is_valid_uv_coord(_mouse_pos_map):
				GameSingleton.set_selected_province(
					GameSingleton.get_province_index_from_uv_coords(_mouse_pos_map)
				)
			else:
		...
		if _mouse_over_viewport:
			if _map_mesh.is_valid_uv_coord(_mouse_pos_map):
				...
				GameSingleton.set_viewed_country_by_province_index(
					GameSingleton.get_province_index_from_uv_coords(_mouse_pos_map)
				)
			else:
...
func _edge_scrolling_vector() -> Vector2:
	if not _mouse_over_viewport:
		return Vector2()
	var mouse_vector := (
		_mouse_pos_viewport * GuiScale.get_current_guiscale() / _viewport_dims - Vector2(0.5, 0.5)
	)
	# Only scroll if outside the move threshold.
	if (
		abs(mouse_vector.x) < 0.5 - _edge_move_threshold
...
func _clamp_over_map() -> void:
	_camera.position.x = (
		_map_mesh_corner.x + fposmod(_camera.position.x - _map_mesh_corner.x, _map_mesh_dims.x)
	)
	_camera.position.z = clamp(
		_camera.position.z, _map_mesh_corner.y, _map_mesh_corner.y + _map_mesh_dims.y
	)


func _update_view_states(force_signal: bool) -> void:
	var new_is_parchment_view: bool = (
		_camera.position.y >= _zoom_parchment_threshold - _zoom_epsilon
	)
...
func _zoom_process(delta: float) -> void:
	...
	var parchment_mapmode: bool = (
		GameSingleton.is_parchment_mapmode_allowed() and _is_parchment_view
	)
	_map_shader_material.set_shader_parameter(
		GameLoader.ShaderManager.param_parchment_mix, float(parchment_mapmode)
	)
...
func _update_orientation() -> void:
	...
	if _is_detailed_view:
		...
		var delta: float = (
			(_zoom_detailed_threshold - _camera.position.y) / _zoom_detailed_threshold
		)
		dir.z = -(delta ** 2)
