...
func _ready() -> void:
	...
	var politics_menu : Panel = GUINode.get_panel_from_nodepath(^"./country_politics")
	...
	_voters_ideology_chart = GUINode.get_gui_pie_chart_from_nodepath(politics_menu.get_node(^"./chart_voters_ideologies"))
	_peoples_ideology_chart = GUINode.get_gui_pie_chart_from_nodepath(politics_menu.get_node(^"./chart_people_ideologies"))

	# Upper House
	_upperhouse_chart = GUINode.get_gui_pie_chart_from_nodepath(politics_menu.get_node(^"./chart_upper_house"))
	...
	# Decisions
	var decisions_window : Panel = GUINode.get_panel_from_nodepath(politics_menu.get_node(^"./decision_window"))
	if decisions_window :
		_decisions_listbox = GUINode.get_gui_listbox_from_node(decisions_window.get_node(^"./decision_listbox"))

	# Movements
	var movements_window : Panel = GUINode.get_panel_from_nodepath(politics_menu.get_node(^"./movements_window"))
	if movements_window :
		...
		if _sortby_size_button  :
			_sortby_size_button.connect(func() -> void: print("MOVEMENTS LIST SORTED BY MOVEMENT SIZE"))
		...
		if _sortby_radical_button  :
			_sortby_radical_button.connect(func() -> void: print("MOVEMENTS LIST SORTED BY MOVEMENT RADICALISM"))
		...
		if _sortby_name_button  :
			_sortby_name_button.connect(func() -> void: print("MOVEMENTS LIST SORTED BY MOVEMENT NAME"))
	...
	var reforms_window : Panel  = GUINode.get_panel_from_nodepath(politics_menu.get_node(^"./reforms_window"))

	# Unciv Reforms
	var unciv_reforms_window : Panel = GUINode.get_panel_from_nodepath(politics_menu.get_node(^"./unciv_reforms_window"))
	if unciv_reforms_window :
		...
		if _westernize_button :
			_westernize_button.connect(func() -> void: print("COUNTRY HAS WESTERNIZED"))

	# Release Nations
	var release_nations_window : Panel = GUINode.get_panel_from_nodepath(politics_menu.get_node(^"./release_nation"))
