...
func set_time(times : PackedFloat32Array) -> void:
	...
	if do_loop_legal or do_loop_illegal:
		for i : int in ages.size():
			var type : int = multimesh.buffer[TR_CUSTOM_SIZE*i + 12] as int
			var start_time := multimesh.buffer[TR_CUSTOM_SIZE*i + 13] 
			...
			if do_loop_legal and type == legal_index and start_time > 0:
				multimesh.buffer[TR_CUSTOM_SIZE*i + 13] = start_time-loop_time_legal
			elif do_loop_illegal and type == illegal_index and start_time > 0:
				multimesh.buffer[TR_CUSTOM_SIZE*i + 13] = start_time-loop_time_illegal
