...
enum MapModes {
	REVOLT_RISK = 2, INFRASTRUCTURE = 5, COLONIAL = 6, NATIONAL_FOCUS = 9, RGO_OUTPUT = 10
}
...
func set_capitals() -> void:
	...
	for capital_index: int in capitals_size:
		...
		multimesh.set_instance_custom_data(capital_index, Color(image_index, 1.0, 0.0, 0.0))
	...
	for capital_index: int in range(capitals_size, total_capitals_size):
		multimesh.set_instance_custom_data(capital_index, Color(image_index, 0.0, 0.0, 0.0))
