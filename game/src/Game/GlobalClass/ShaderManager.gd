...
func _set_shader_texture(
	...
	texture_param: StringName,
	texture: Texture,
	tile_factor_param: StringName = &"",
	pixels_per_tile: float = 0.0
) -> Error:
	var err: Error = OK
	if texture != null:
...
func _set_shader_asset_texture(
	...
	return _set_shader_texture(
		shader_material,
		texture_param,
		AssetManager.get_texture(texture_path),
		tile_factor_param,
		pixels_per_tile
	)
...
func set_up_shader(material: Material, add_cosmetic_textures: bool) -> Error:
	...
	if (
		_set_shader_texture(
			shader_material, param_province_shape_tex, GameSingleton.get_province_shape_texture()
		)
		!= OK
	):
	...
	if subdivisions.x >= 1 and subdivisions.y >= 1:
		shader_material.set_shader_parameter(
			param_province_shape_subdivisions, Vector2(subdivisions)
		)
	else:
		push_error(
			"Invalid province shape image subdivision: ", subdivisions.x, "x", subdivisions.y
		)
		ret = FAILED
	...
	if add_cosmetic_textures:
		...
		if (
			_set_shader_texture(
				...
				GameSingleton.get_province_colour_texture()
			)
			!= OK
		):
		...
		if (
			_set_shader_texture(
				shader_material,
				param_terrain_tex,
				GameSingleton.get_terrain_texture(),
				param_terrain_tile_factor,
				pixels_per_terrain_tile
			)
			!= OK
		):
		...
		if (
			_set_shader_asset_texture(
				shader_material,
				param_stripe_tex,
				&"map/terrain/stripes.dds",
				param_stripe_tile_factor,
				pixels_per_stripe_tile
			)
			!= OK
		):
		...
		if (
			_set_shader_asset_texture(
				shader_material,
				param_overlay_tex,
				&"map/terrain/map_overlay_tile.dds",
				param_overlay_tile_factor,
				pixels_per_overlay_tile
			)
			!= OK
		):
		...
		if (
			_set_shader_asset_texture(
				shader_material, param_colormap_land_tex, &"map/terrain/colormap.dds"
			)
			!= OK
		):
		...
		if (
			_set_shader_asset_texture(
				shader_material, param_colormap_water_tex, &"map/terrain/colormap_water.dds"
			)
			!= OK
		):
		...
		if (
			_set_shader_asset_texture(
				shader_material, param_colormap_overlay_tex, &"map/terrain/colormap_political.dds"
			)
			!= OK
		):
