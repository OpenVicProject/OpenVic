...
func _set_shader_texture(
	...
	if tile_factor_param:
		# Set to 1.0 / pixels_per_tile as the shader can multiply faster than it can divide, and it
		# will not automatically
		# optimise to multiplication by a reciprocal for fear of losing precision. As
		# pixels_per_tile is often a power of two,
...
func set_up_shader(material: Material, add_cosmetic_textures: bool) -> Error:
	...
	if subdivisions.x >= 1 and subdivisions.y >= 1:
		shader_material.set_shader_parameter(
			...
			Vector2(subdivisions),
		)
	else:
		push_error(
			"Invalid province shape image subdivision: ",
			subdivisions.x,
			"x",
			subdivisions.y,
		)
