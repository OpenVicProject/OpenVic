...
var _saved_ips_file_path: String = ProjectSettings.get_setting(
	ips_file_path_setting,
	ips_file_path_default,
)
...
func load_ips_config_file() -> ConfigFile:
	...
	if FileAccess.file_exists(override_path):
		if _saved_ips_file.load(override_path) != OK:
			...
			if not _saved_ips_file.has_section_key(USER, PLAYER_NAME):
				_saved_ips_file.set_value(USER, PLAYER_NAME, DEFAULT_PLAYER_NAME)
			if not _saved_ips_file.has_section_key(USER, HOST_GAME_NAME):
				_saved_ips_file.set_value(USER, HOST_GAME_NAME, DEFAULT_GAME_NAME)
			if not _saved_ips_file.has_section_key(USER, HOST_GAME_PASSWORD):
