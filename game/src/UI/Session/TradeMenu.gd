...
func _update_info() -> void:
	if _active:
		...
		var good_producers_tooltips: PackedStringArray = trade_info.get(
			good_producers_tooltips_key,
			[] as PackedStringArray,
		)

		_generate_listbox(
			Table.GOVERNMENT_NEEDS,
			trade_info.get(government_needs_key, [] as PackedVector2Array),
			good_producers_tooltips,
		)
		_generate_listbox(
			Table.FACTORY_NEEDS,
			trade_info.get(factory_needs_key, [] as PackedVector2Array),
			good_producers_tooltips,
		)
		_generate_listbox(
			Table.POP_NEEDS,
			trade_info.get(pop_needs_key, [] as PackedVector2Array),
			good_producers_tooltips,
		)
		_generate_listbox(
			Table.MARKET_ACTIVITY,
			trade_info.get(market_activity_key, [] as PackedVector3Array),
			good_producers_tooltips,
		)
		_generate_listbox(
			Table.STOCKPILE,
			trade_info.get(stockpile_key, [] as PackedVector3Array),
			trade_info.get(good_trading_yesterday_tooltips_key, [] as PackedStringArray),
		)
		_generate_listbox(
			Table.COMMON_MARKET,
			trade_info.get(common_market_key, [] as PackedVector4Array),
			good_producers_tooltips,
		)
...
# Set new_trade_detail_good_index to -1 if you're updating for any reason other than explicitly
# selecting a trade good to view the details of
...
func _update_trade_details(new_trade_detail_good_index: int = -1) -> void:
	...
	# Even if the new index is the same as the current index, it indicates a forced refresh
	# (including the trade order
	...
	var price_history: PackedFloat32Array = trade_info.get(
		trade_detail_price_history_key,
	...
	if _trade_detail_government_good_needs_label:
		_trade_detail_government_good_needs_label.add_substitution(
			"VAL",
			GUINode.float_to_string_dp(trade_info.get(trade_detail_government_needs_key, 0), 2),
		)
	...
	if _trade_detail_pop_good_needs_label:
		_trade_detail_pop_good_needs_label.add_substitution(
			"VAL",
			GUINode.float_to_string_dp(trade_info.get(trade_detail_pop_needs_key, 0), 2),
		)

	if _trade_detail_good_available_label:
		_trade_detail_good_available_label.add_substitution(
			"VAL",
			GUINode.float_to_string_dp(trade_info.get(trade_detail_available_key, 0), 2),
		)
...
func _generate_listbox(table: Table, data: Variant, good_tooltips: PackedStringArray) -> void:
	...
	for index in min(listbox.get_child_count(), data.size()):
		...
		var good_button: GUIIconButton = GUINode.get_gui_icon_button_from_node_and_path(
			entry,
			item_paths[0],
		)
		if good_button:
...
func _generate_good_entries(good_tooltips: PackedStringArray) -> void:
	...
	for good_category: String in good_categories_info:
		...
		var max_items_per_row: int = max(
			floor(good_category_panel.get_size().x / _goods_entry_offset.x),
			1,
		)
		...
		while child_count < good_category_goods.size():
			...
			good_entry_panel.set_position(
				_goods_entry_offset * Vector2(
					child_count % max_items_per_row,
					child_count / max_items_per_row,
				)
			)
		...
		for category_index: int in min(child_count, good_category_goods.size()):
			...
			var entry_button: GUIIconButton = get_gui_icon_button_from_node_and_path(
				good_entry_panel,
				^"./entry_button",
			)
			if entry_button:
				# Connecting this way ensures the Callable is always the same, preventing errors
				# when good_index changes,
			...
			var good_button: GUIIconButton = get_gui_icon_button_from_node_and_path(
				good_entry_panel,
				^"./goods_type",
			)
			if good_button:
			...
			var price_label: GUILabel = get_gui_label_from_node_and_path(
				good_entry_panel,
				^"./price",
			)
			if price_label:
			...
			var trend_icon: GUIIcon = get_gui_icon_from_node_and_path(
				good_entry_panel,
				^"./trend_indicator",
			)
			if trend_icon:
			...
			var automated_icon: GUIIcon = get_gui_icon_from_node_and_path(
				good_entry_panel,
			...
			var buy_sell_icon: GUIIcon = get_gui_icon_from_node_and_path(
				good_entry_panel,
				^"./selling_indicator",
			)
			if buy_sell_icon:
