...
func _ready() -> void:
	...
	_tech_areas = _tech_defines.get("tech_areas", [] as Array[PackedStringArray])
	...
	for i : int in _tech_folders.size():
		...
		var pos : Vector2 = GUINode.get_gui_position("country_technology", "folder_offset")
		...
		var title : GUILabel = GUINode.get_gui_label_from_node(folder_node.get_node(^"./folder_category"))
		if title:
			title.set_text(_tech_folders[i])

		var button : GUIIconButton = GUINode.get_gui_icon_button_from_node(folder_node.get_node(^"./folder_button"))
		...
		var progressbar : GUIProgressBar = GUINode.get_gui_progress_bar_from_node(folder_node.get_node(^"./folder_progress"))
		if progressbar:
			progressbar.mouse_filter = Control.MOUSE_FILTER_IGNORE
			_tech_folder_progressbars.push_back(progressbar)

		var discovered : GUILabel = GUINode.get_gui_label_from_node(folder_node.get_node(^"folder_number_discovered"))
		if discovered:
			_tech_folder_number_discovered_labels.push_back(discovered)

		# areas
		var folder_areas : PackedStringArray = _tech_areas[i]
		for area_index : int in range(folder_areas.size()):
			...
			var area_title : GUILabel = GUINode.get_gui_label_from_node(area_node.get_node(^"./group_name"))
			if area_title:
			...
			for tech_index : int in range(area_technologies.size()):
				...
				var tech_name : GUILabel = GUINode.get_gui_label_from_node(tech_node.get_node(^"./tech_name"))
				if tech_name:
					tech_name.set_text(area_technologies[tech_index])

				var tech_button : GUIIconButton = GUINode.get_gui_icon_button_from_node(tech_node.get_node(^"./start_research"))
				if tech_button:
