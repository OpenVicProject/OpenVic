...
func _ready() -> void:
	...
	# TODO: add keyboard shortcuts (and shortcut tooltips) where vanilla does by default + use key
	...
	# This will set the mapmode in GameSingleton which in turn updates the buttons so that the right
	# one is pressed
	_mapmode_pressed(0)
...
func _on_mapmode_changed(index: int) -> void:
	...
	if current_mapmode_button != new_mapmode_button:
		if new_mapmode_button:
			...
			# current_mapmode_button can't be null as it isn't equal to new_mapmode_button which is
			# null
