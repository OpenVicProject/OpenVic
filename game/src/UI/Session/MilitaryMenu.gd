...
func _update_unit_group_list(
	table: Table,
	unit_groups: Array[Dictionary],
	in_progress_units: Array[Dictionary],
) -> void:
	...
	for index: int in mini(unit_groups.size(), listbox.get_child_count()):
		...
		if leader_icon:
			var leader_texture: Texture2D = unit_group_dict.get(
		...
		if is_army:
			var men_count: int = unit_group_dict.get(
				military_info_unit_group_men_count_key,
				0,
			) if is_army else 0
			var max_men_count: int = unit_group_dict.get(
				military_info_unit_group_max_men_count_key,
				0,
			) if is_army else 0
			var men_count_string: String = GUINode.int_to_string_commas(men_count) if is_army else ""
			strength_tooltip = strength_tooltip.replace(
				"$VALUE$",
				men_count_string,
			).replace("$MAX$", GUINode.int_to_string_commas(max_men_count))
		...
		if organisation_progress_bar:
			...
			var organisation: float = unit_group_dict.get(
				military_info_unit_group_organisation_key,
				0.0,
			)
		...
		if moving_icon:
			var moving_tooltip: String = unit_group_dict.get(
				military_info_unit_group_moving_tooltip_key,
				"",
			)
		...
		if is_army:
			if dig_in_icon:
				var dig_in_tooltip: String = unit_group_dict.get(
					military_info_unit_group_dig_in_tooltip_key,
					"",
				)
	...
	for index: int in clampi(
		listbox.get_child_count() - unit_groups.size(),
		0,
		in_progress_units.size(),
	):
...
func _update_info() -> void:
	if _active:
			...
			_unit_start_experience_label.set_text(
				"%s/%s" % [
					GUINode.float_to_string_dp(
						military_info.get(military_info_land_unit_start_experience_key, 0),
						2,
					),
					GUINode.float_to_string_dp(
						military_info.get(military_info_naval_unit_start_experience_key, 0),
						2,
					),
				]
			)
		...
		var mobilisation_size_tooltip: String = military_info.get(
			military_info_mobilisation_size_tooltip_key,
			"",
		)
		var mobilisation_impact_tooltip: String = military_info.get(
			military_info_mobilisation_impact_tooltip_key,
			"",
		)
		if _mobilise_button:
		...
		var mobilisation_progress: float = military_info.get(
			military_info_mobilisation_progress_key,
			0,
		)
		var mobilisation_progress_string: String = GUINode.float_to_string_dp(
			100 * mobilisation_progress,
			2,
		)
		...
		# TODO - leave the main text set and only update $VALUE$ using substitution dict
		# functionality once buttons get proper text support
		if _create_general_button:
		...
		if leader_listbox:
			...
			while leader_listbox.get_child_count() < leader_entries.size():
				var unit_entry: Panel = GUINode.generate_gui_element(
					_gui_file,
					"milview_leader_entry",
				)
				if not unit_entry:
					break
			...
			for index: int in mini(leader_entries.size(), leader_listbox.get_child_count()):
				...
				if leader_id == 0:
					push_error(
						"Leader ID is 0 or missing in leader dictionary for entry index ",
						index,
						", skipping!",
					)
					continue
				...
				entry_menu.set_meta(
					TABLE_COLUMN_KEYS[1],
					leader_dict.get(military_info_leader_branch_key, 0),
				)
				...
				var leader_icon: GUIIcon = GUINode.get_gui_icon_from_node(entry_menu.get_node(^"./leader"))
				if leader_icon:
					var leader_texture: Texture2D = leader_dict.get(
						military_info_leader_picture_key,
						null,
					)
					if leader_texture:
						leader_icon.set_texture(leader_texture)
				...
				if use_leader_button:
					...
					use_leader_button.toggled.connect(
						func(state: bool) -> void:
							PlayerSingleton.set_can_use_leader(
								entry_menu.get_meta(military_info_leader_id_key, 0) as int,
								state,
							)
					)
				...
				var leader_assignment_untype: Variant = leader_dict.get(
					military_info_leader_assignment_key,
					null,
				)
				if leader_assignment_untype != null:
					...
					var leader_location: String = GUINode.format_province_name(
						leader_dict.get(military_info_leader_location_key, ""),
						true,
					)
...
func _make_unsortable_check(callable: Callable) -> Callable:
	return func(a: Node, b: Node) -> bool:
		...
		return not a.get_meta(
			TABLE_UNSORTABLE_KEY,
			false,
		) and (b.get_meta(TABLE_UNSORTABLE_KEY, false) or callable.call(a, b))
