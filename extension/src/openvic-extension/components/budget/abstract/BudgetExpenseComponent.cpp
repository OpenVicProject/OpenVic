...
BudgetExpenseComponent::BudgetExpenseComponent(
    godot::StringName&& new_expenses_summary_localisation_key, const int32_t new_expenses_summary_decimal_places
) :
...
godot::String BudgetExpenseComponent::generate_expenses_summary_text(godot::Object const& translation_object) const {
	const float expenses = static_cast<float>(get_expenses());
	return translation_object.tr(expenses_summary_localisation_key)
	    .replace(
	        Utilities::get_short_value_placeholder(),
	        expenses_summary_decimal_places < 0 ? Utilities::float_to_string_dp_dynamic(expenses)
...
godot::String BudgetExpenseComponent::generate_balance_expenses_summary_text(godot::Object const& translation_object) const {
	...
	return translation_object.tr(expenses_summary_localisation_key)
	    .replace("Y$VAL$", red_minus + Utilities::float_to_string_dp(expenses, expenses_summary_decimal_places));
}
