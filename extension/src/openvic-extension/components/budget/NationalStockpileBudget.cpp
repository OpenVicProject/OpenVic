...
NationalStockpileBudget::NationalStockpileBudget(GUINode const& parent)
	: BudgetExpenseComponent("BUDGET_IMPORTS"),
	  ...
	  army_stockpile_budget { parent }, navy_stockpile_budget { parent }, construction_stockpile_budget { parent } {
	...
	connections[2] =
		construction_stockpile_budget.balance_changed.connect(&NationalStockpileBudget::on_slider_value_changed, this);

	GUILabel::set_text_and_tooltip(parent, "./country_budget/nat_stock_desc", "BUDGET_NATIONAL_STOCKPILE", "NAT_STOCK_DESC");
}
...
void NationalStockpileBudget::update_labels(CountryInstance& country) {
	const fixed_point_t military_balance = army_stockpile_budget.get_balance() + navy_stockpile_budget.get_balance();
	const fixed_point_t balance = military_balance + construction_stockpile_budget.get_balance();

	military_costs_label.set_text(Utilities::cash_to_string_dp_dynamic(-military_balance));
	const fixed_point_t actual_stockpile_spending = country.get_actual_national_stockpile_spending();
	todays_actual_stockpile_spending_label.set_text(Utilities::cash_to_string_dp_dynamic(actual_stockpile_spending));
	...
	if (actual_stockpile_spending > 0) {
		todays_actual_stockpile_spending_tooltip += "\n"; // This is for a whiteline, not just a linebreak. Just like
														  // Victoria 2.
		...
		std::ranges::sort(bought_goods, [](auto const& lhs, auto const& rhs) {
	...
	expenses_label.set_text(Utilities::cash_to_string_dp_dynamic(-balance));
