...
SocialBudget::SocialBudget(GUINode const& parent) :
    SliderBudgetComponent(
        parent, "BUDGET_SLIDER_SOCIAL_SPENDING", EXPENSES, "./country_budget/exp_2_slider", "./country_budget/exp_val_2"
    ),
    BudgetExpenseComponent("BUDGET_SLIDER_SOCIAL_SPENDING"),
    pensions_label {
	    *parent.get_gui_label_from_nodepath("./country_budget/exp_val_2")
    }, // TODO figure out how to get their path
	...
	GUILabel::set_text_and_tooltip(parent, "./country_budget/soc_stand_desc", "SOCIAL_SPENDING", "SOCIAL_DESC2");
}
...
fixed_point_t SocialBudget::calculate_budget_and_update_custom(CountryInstance& country, const fixed_point_t scaled_value) {
	...
	pensions_label.set_text(Utilities::cash_to_string_dp_dynamic(pensions));
	const fixed_point_t unemployment_subsidies =
	    scaled_value * country.get_projected_unemployment_subsidies_spending_unscaled_by_slider_untracked();
	unemployment_subsidies_label.set_text(Utilities::cash_to_string_dp_dynamic(unemployment_subsidies));
