...
AdministrationBudget::AdministrationBudget(GUINode const& parent, CountryDefines const& new_country_defines) :
    SliderBudgetComponent(
        parent, "BUDGET_SLIDER_ADMINISTRATION", EXPENSES, "./country_budget/exp_1_slider", "./country_budget/exp_val_1"
    ),
    BudgetExpenseComponent("BUDGET_SLIDER_ADMINISTRATION"), country_defines { new_country_defines },
	...
	GUILabel::set_text_and_tooltip(parent, "./country_budget/admin_desc", "ADMINISTRATION", "ADM_DESC");
...
fixed_point_t AdministrationBudget::calculate_budget_and_update_custom(
	...
	administrative_efficiency_label.set_text(
	    Utilities::format("%s%%", Utilities::fixed_point_to_string_dp(100 * administrative_efficiency_from_administrators, 1))
	);

	administrative_efficiency_tooltip_args[0] =
	    administrative_efficiency_label.tr("BUDGET_ADMIN_EFFICIENCY2")
	        .replace(
	...
	if (scaled_value == 0) {
		...
		administrative_efficiency_tooltip_args[2] = Utilities::format(
		    admin_crime_template, administrative_efficiency_label.tr("BUDGET_VIEW_CRIME_FIGHT"),
		    Utilities::fixed_point_to_string_dp(100 * admin_spending_crime_effect * scaled_value, 1),
		    administrative_efficiency_label.tr("PV_CRIMEFIGHT_MAX")
		        .replace(
		            Utilities::get_percentage_value_placeholder(),
	...
	administrative_efficiency_tooltip_args[3] =
	    administrative_efficiency_label.tr("BUDGET_ADMIN_EFFICIENCY_DESC")
	        .replace(
	            Utilities::get_percentage_value_placeholder(),
	            Utilities::fixed_point_to_string_dp(100 * country.get_administrator_percentage_untracked(), 1)
	        )
	        .replace(
	            "MAX", Utilities::fixed_point_to_string_dp(100 * country.desired_administrator_percentage.get_untracked(), 1)
	        );
	administrative_efficiency_tooltip_args[5] =
	    administrative_efficiency_label.tr("COMWID_BASE")
	        .replace(
	...
	for (auto const& [group, reform] : country.get_reforms()) {
		...
		const fixed_point_t extra_administrator_percentage =
		    administrative_multiplier * country_defines.get_bureaucracy_percentage_increment();
		reforms_part += Utilities::format(
		    reform_template, administrative_efficiency_label.tr(convert_to<godot::String>(group.get_identifier())),
		    administrative_efficiency_label.tr(convert_to<godot::String>(reform->get_identifier())),
	...
	if (budget_label != nullptr) {
		budget_label->set_tooltip_string(
		    Utilities::format(
		        "%s\n--------------\n%s%s", budget_label->tr("DIST_ADMINISTRATION"),
		        was_budget_cut(country)
		            ? budget_label->tr("EXPENSE_NO_AFFORD")
		                      .replace(
		                          Utilities::get_short_value_placeholder(),
