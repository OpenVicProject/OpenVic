...
StrataTaxBudget::StrataTaxBudget(
    GUINode const& parent, Strata const& new_strata, ModifierEffectCache const& new_modifier_effect_cache
) :
    SliderBudgetComponent(
        parent, generate_slider_tooltip_localisation_key(new_strata), BALANCE,
    ...
    BudgetIncomeComponent(generate_summary_localisation_key(new_strata), 1), strata { new_strata },
	...
	GUILabel::set_text_and_tooltip(
	    parent, Utilities::format("./country_budget/tax_%d_desc", static_cast<uint64_t>(type_safe::get(new_strata.index))),
	    generate_slider_tooltip_localisation_key(new_strata),
	    Utilities::format("TAX_%s_DESC", convert_to<godot::String>(strata.get_identifier()).to_upper())
	);
}
...
fixed_point_t StrataTaxBudget::calculate_budget_and_update_custom(CountryInstance& country, const fixed_point_t scaled_value) {
	return scaled_value * country.get_taxable_income_by_strata(strata) * country.get_tax_efficiency_untracked();
}
...
void StrataTaxBudget::on_slider_value_changed(const fixed_point_t scaled_value) {
	PlayerSingleton::get_singleton()->set_strata_tax_rate_slider_value(strata, scaled_value);
}
...
void StrataTaxBudget::update_slider_tooltip(CountryInstance& country, const fixed_point_t scaled_value) {
	// these use $VALUE$%
	...
	const godot::String tax_efficiency_text = slider.tr(tax_efficiency_localisation_key)
	                                              .replace(
	...
	const godot::String tax_efficiency_from_tech_text =
	    slider.tr(tax_efficiency_from_tech_localisation_key)
	        .replace(
	...
	const godot::String effective_tax_rate_text =
	    slider.tr(effective_tax_rate_localisation_key)
	        .replace(
	            Utilities::get_long_value_placeholder(),
	...
	const godot::String tooltip = Utilities::format(
	    godot::String::utf8("%s: §Y%s§!\n%s\n%s\n%s"), localised_strata_tax,
	    Utilities::percentage_to_string_dp(scaled_value, 1), tax_efficiency_text, tax_efficiency_from_tech_text,
