...
DiplomaticBudget::DiplomaticBudget(GUINode const& parent) :
	...
	GUILabel::set_text_and_tooltip(parent, "./country_budget/diplomatic_desc", "DIPLOMATIC_BALANCE_DESC", "BUDGET_DIPL_DESC");
}
...
#define TOOLTIP_TEXT_PART(value, localisation_key) \
	value > 0 ? balance_label.tr(localisation_key) \
	                .replace(Utilities::get_short_value_placeholder(), godot::String::num_int64(value.round<int32_t>())) \
	          : ""
...
void DiplomaticBudget::full_update(CountryInstance& country) {
	...
	const fixed_point_t diplomatic_balance =
	    reparations_income + war_subsidies_income - reparations_expenses - war_subsidies_expenses;
	set_balance(diplomatic_balance);

	balance_label.set_text(
	    Utilities::format(
	        godot::String::utf8("§%s%s§!"), Utilities::get_colour_and_sign(diplomatic_balance),
...
godot::String DiplomaticBudget::generate_expenses_summary_text(godot::Object const& translation_object) const {
	...
	return reparations_expenses > 0
	         ? "\n" + translation_object.tr(expenses_summary_localisation_key)
	                      .replace(
	                          Utilities::get_short_value_placeholder(),
	                          Utilities::float_to_string_dp(
	                              static_cast<float>(reparations_expenses), expenses_summary_decimal_places
	                          )
	                      )
	         : "";
}
