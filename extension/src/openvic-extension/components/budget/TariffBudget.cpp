...
TariffBudget::TariffBudget(GUINode const& parent, CountryDefines const& country_defines)
	: SliderBudgetComponent(
		  parent, "TARIFFS_INCOME", BALANCE, "./country_budget/tariff_slider", "./country_budget/tariff_val",
	...
	GUILabel::set_text_and_tooltip(parent, "./country_budget/tariffs", "BUDGET_TARIFFS", "TARIFFS_DESC");
	slider_tooltip_args.resize(4);
	slider_tooltip_args[2] = parent.tr("BASE_TARIFF_EFFICIENCY")
								 .replace(
									 Utilities::get_percentage_value_placeholder(),
									 // not percentage_to_string_dp as localisation text contains the %
									 Utilities::fixed_point_to_string_dp(100 * country_defines.get_base_tariff_efficiency(), 1)
								 );
}
...
fixed_point_t TariffBudget::calculate_budget_and_update_custom(CountryInstance& country, const fixed_point_t scaled_value) {
	return scaled_value * country.get_yesterdays_import_value_untracked() * country.tariff_efficiency.get_untracked();
}
...
void TariffBudget::update_slider_tooltip(CountryInstance& country, const fixed_point_t scaled_value) {
	...
	godot::StringName const& prefix = scaled_value < 0 ? red : green;

	slider_tooltip_args[0] =
		slider.tr(slider_tooltip_localisation_key)
			.replace(
				"Y" + Utilities::get_short_value_placeholder(), prefix + Utilities::percentage_to_string_dp(scaled_value, 1)
			);

	slider_tooltip_args[1] =
		slider.tr("BUDGET_TARIFFS_MODIFIED")
			.replace(
				"Y" + Utilities::get_percentage_value_placeholder(),
				// not percentage_to_string_dp as localisation text contains the %
				prefix + Utilities::fixed_point_to_string_dp(100 * scaled_value * country.tariff_efficiency.get_untracked(), 1)
			);

	slider_tooltip_args[3] = slider.tr("BUDGET_ADMIN_EFFICIENCY")
								 .replace(
									 Utilities::get_percentage_value_placeholder(),
									 // not percentage_to_string_dp as localisation text contains the %
