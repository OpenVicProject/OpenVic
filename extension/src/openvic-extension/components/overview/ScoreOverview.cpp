...
{
	on_gamestate_updated();
}

void ScoreOverview::on_gamestate_updated() {
	disconnect_all();
	GameSingleton& game_singleton = *GameSingleton::get_singleton();
	game_singleton.gamestate_updated.connect(&ScoreOverview::on_gamestate_updated, this);
	...
	if (!country.can_colonise()) {
		...
		ModifierEffect const* const colonial_points_modifier = instance_manager.get_definition_manager().get_modifier_manager().get_modifier_effect_cache().get_colonial_points();
		const godot::String from_tech_part = colonial_power_label.tr("FROM_TECHNOLOGY")+": "+ MenuSingleton::_make_modifier_effect_value_coloured(
