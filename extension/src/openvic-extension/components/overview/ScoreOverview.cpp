...
void ScoreOverview::update() {
	...
	on_prestige_changed(country.get_prestige([this](signal<fixed_point_t>& prestige_changed) mutable -> void {
	...
	on_industrial_score_changed(
		country.get_industrial_power([this](signal<fixed_point_t>& industrial_score_changed) mutable -> void {
	...
	update_military_score(country.military_power.get([this](signal<>& military_score_changed) mutable -> void {
	...
	if (!country.can_colonise()) {
		colonial_power_tooltip = Utilities::format(
			"%s%s%s", colonial_power_label.tr("COLONIAL_POINTS"), MenuSingleton::get_tooltip_separator(),
		...
		const godot::String available_part =
			colonial_power_label.tr("AVAILABLE_COLONIAL_POWER")
				.replace(Utilities::get_long_value_placeholder(), godot::String::num_int64(available_colonial_power));

		InstanceManager const& instance_manager = *game_singleton.get_instance_manager();
		ModifierEffect const* const colonial_points_modifier =
			instance_manager.definition_manager.get_modifier_manager().get_modifier_effect_cache().get_colonial_points();
		const godot::String from_tech_part =
			colonial_power_label.tr("FROM_TECHNOLOGY") + ": " +
			Utilities::make_modifier_effect_value_coloured(
				colonial_power_label, *colonial_points_modifier, colonial_power_from_technology, false
			);
		...
		const godot::String investment_part = colonial_power_invested == 0 ? ""
				...
				colonial_power_label.tr("COLONIAL_INVESTMENT")
					.replace(
						Utilities::get_long_value_placeholder(), Utilities::fixed_point_to_string_dp(colonial_power_invested, 1)
					);
		...
		colonial_power_tooltip = Utilities::format(
			"%s%s%s\n%s%s%s", colonial_power_label.tr("COLONIAL_POINTS"), MenuSingleton::get_tooltip_separator(),
			available_part, from_tech_part, investment_part, maintenance_part
		);
	}
	...
	//  		topbar_info.get(colonial_power_tooltip_key, "") if country_status <= CountryStatus.SECONDARY_POWER else
	//  tr(&"NON_COLONIAL_POWER")
	//  	))
}
...
godot::String ScoreOverview::generate_prestige_tooltip(CountryInstance& country) {
	...
	return prestige_label.tr("RANK_PRESTIGE") + prestige_tooltip + MenuSingleton::get_tooltip_separator() +
...
godot::String ScoreOverview::generate_industrial_tooltip(CountryInstance& country) {
	...
	for (auto const& [state, power] : country.get_industrial_power_from_states()) {
		industrial_power_states.emplace_back(Utilities::get_state_name(industrial_score_label, state), power);
	}
	...
	for (auto const& [state_name, power] : industrial_power_states) {
		...
		industrial_power_tooltip +=
			Utilities::format(state_power_template_string, state_name, Utilities::fixed_point_to_string_dp(power, 3));
	}
	...
	for (auto const& [investor_ref, power] : country.get_industrial_power_from_investments()) {
		...
		industrial_power_from_investments.emplace_back(
			convert_to<godot::String>(investor.get_identifier()), Utilities::get_country_name(industrial_score_label, investor),
			power
		);
	}
	std::sort(
	...
	for (auto const& [country_identifier, country_name, power] : industrial_power_from_investments) {
		...
		industrial_power_tooltip += Utilities::format(
			investment_power_template_string, country_identifier, country_name, Utilities::fixed_point_to_string_dp(power, 3)
		);
	}

	return industrial_score_label.tr("RANK_INDUSTRY") + MenuSingleton::get_tooltip_separator() +
		industrial_score_label.tr("RANK_INDUSTRY_D") + industrial_power_tooltip;
}

void ScoreOverview::on_prestige_changed(const fixed_point_t new_prestige) {
	prestige_label.set_text(Utilities::fixed_point_to_string_dp(new_prestige, 0));
}
void ScoreOverview::on_industrial_score_changed(const fixed_point_t new_industrial_score) {
	industrial_score_label.set_text(Utilities::fixed_point_to_string_dp(new_industrial_score, 0));
}
...
godot::String ScoreOverview::generate_military_tooltip(CountryInstance& country) {
	...
	return military_score_label.tr("RANK_MILITARY") + MenuSingleton::get_tooltip_separator() +
		military_score_label.tr("RANK_MILITARY_D") + military_power_tooltip;
}
...
void ScoreOverview::update_military_score(const fixed_point_t new_military_score) {
	military_score_label.set_text(Utilities::fixed_point_to_string_dp(new_military_score, 0));
}
