...
namespace OpenVic {
	...
	static bool read_skinning(Ref<FileAccess> const& file, skinning_t& skin, std::vector<mesh_t>& meshes, int32_t version) {
	...
	*/
	...
	static std::vector<material_mapping> build_materials(std::vector<material_definition_t> const& materials) {
		...
		for(material_definition_t mat : materials) {
			String diffuse_name;
			String specular_name;
			String normal_name;

			for(material_layer_t layer : mat.layers) {
	...
	*/
	...
	static MeshInstance3D* build_mesh(mesh_t const& mesh_chunk, skinning_t* skin, std::vector<material_mapping> const& materials) {
		...
		for(vertices_attribute_t attribute : mesh_chunk.vertices_attributes) {
		...
		for(submesh_t submesh : mesh_chunk.submeshes) {
			...
			for(uint32_t rel_ind : submesh.relative_indices) {
				...
				if (normals.size() > index) {
					normals_submesh.push_back( vec3d_to_godot(normals[index]));
				}
