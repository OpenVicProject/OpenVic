...
namespace OpenVic {
	...
	struct material_layer_pack { //also chunk 0x4, v2
		float amount;
		vec2d_t uv_offset;
		vec2d_t uv_tiling;
	...
	struct node_chunk_pack {
		...
		int32_t parent_node_id;
		float uncertain[17]; //likely a matrix44 + something else
	};
	...
	static bool read_layer(Ref<FileAccess> const& file, material_layer& layer) {
		bool ret = true;
	...
	static bool read_skinning(Ref<FileAccess> const& file, skinning& skin, std::vector<mesh>& meshes, int32_t version) {
	...
	*/
	...
	static Skeleton3D* build_armature_hierarchy(node_hierarchy const& hierarchy) {
	...
	*/
	...
	static std::vector<material_mapping> build_materials(std::vector<material_definition> const& materials) {
		...
		for(material_definition mat : materials) {
			...
			for(material_layer layer : mat.layers) {
				...
				switch(layer.packed.map_type){
					//Layer type 4 is used by Fort_Early.xac
					// it corresponds to the blob shadow? Fort_Early_Shadow.dds
					// for now, skip
	...
	*/
	...
	template<typename T>
	static void convert_data(std::vector<T>& target, std::vector<uint8_t>& source) {
		target.resize(source.size()/sizeof(T));
		std::memcpy(target.data(),source.data(),source.size());
	}

	static MeshInstance3D* build_mesh(mesh const& mesh_chunk, skinning* skin, std::vector<material_mapping> const& materials) {
		...
		for(vertices_attribute attribute : mesh_chunk.vertices_attributes) {
			switch(attribute.packed.type){
				...
				case ATTRIBUTE::TANGENT:
					...
					break;
				case ATTRIBUTE::UV:
					if(uvs_read == 0) convert_data(uv1, attribute.data);
					else if(uvs_read == 1) convert_data(uv2, attribute.data);
					uvs_read += 1;
					break;
		...
		for(submesh submesh : mesh_chunk.submeshes) {
			...
			if(!verts_submesh.is_empty()) 		arrs[Mesh::ARRAY_VERTEX] = std::move(verts_submesh);
			if(!normals_submesh.is_empty()) 	arrs[Mesh::ARRAY_NORMAL] = std::move(normals_submesh);
			if(!tangents_submesh.is_empty()) 	arrs[Mesh::ARRAY_TANGENT] = std::move(tangents_submesh);
			if(!uv1_submesh.is_empty()) 		arrs[Mesh::ARRAY_TEX_UV] = std::move(uv1_submesh);
			if(!uv2_submesh.is_empty()) 		arrs[Mesh::ARRAY_TEX_UV2] = std::move(uv2_submesh);
