...
struct xsm_metadata_v2_pack_extra {
	float unused;
	float max_acceptable_error;
};

struct xsm_metadata_pack {
	int32_t fps;
...
struct skeletal_submotion_pack {
...
*** This chunk appears a lot in the zulu_moving.xsm animation
zulu moving doesn't have any 0xCA chunks, so this is likely a replacement
for the bone animation chunk...
Let's call it node_submotion
*/

struct xsm_metadata {
	bool has_v2_data = false;
	xsm_metadata_v2_pack_extra v2_data;
	xsm_metadata_pack packed;
	String source_app;
...
static bool read_xsm_metadata(Ref<FileAccess> const& file, xsm_metadata& metadata, int32_t version) {
	bool res = true;
	if(version != 1) {
...
static bool read_rot_keys(Ref<FileAccess> const& file, std::vector<rotation_key>* keys_out, int32_t count, int32_t version) {
	bool res = true;

	if(version == 1) {
		std::vector<rotation_key_v1> rot_keys_v1;
		res &= read_struct_array(file,rot_keys_v1,count);
		for(rotation_key_v1 key : rot_keys_v1) {
			keys_out->push_back({quat_v1_to_godot(key.rotation), key.time});
		}
	}
	else {
		std::vector<rotation_key_v2> rot_keys_v2;
		res &= read_struct_array(file,rot_keys_v2,count);
		for(rotation_key_v2 key : rot_keys_v2) {
...
static bool read_skeletal_submotion(Ref<FileAccess> const& file, skeletal_submotion& submotion, int32_t version){
	bool res = true;
	if(version == 1) { //float component quats (v1)
		submotion_rot_v1_pack rot_comps;
		...
		submotion_rot_v2_pack rot_comps;
		res &= read_struct(file,rot_comps);
...
static bool read_xsm_bone_animation(Ref<FileAccess> const& file, bone_animation& bone_animation, int32_t version){
	bone_animation.submotion_count = file->get_32();
	bool ret = true;

	for(int i=0; i<bone_animation.submotion_count; i++){
		skeletal_submotion submotion;
...
static bool read_node_submotion(Ref<FileAccess> const& file, std::vector<node_submotion>* submotions) {
	bool res = true;
	node_submotion submotion;
...
template<typename T>
float add_submotion(Ref<Animation> anim, T submotion) {
	...
	/*anim->track_set_interpolation_type(pos_id, Animation::INTERPOLATION_LINEAR);
	anim->track_set_interpolation_type(rot_id, Animation::INTERPOLATION_LINEAR);
	anim->track_set_interpolation_type(scale_id, Animation::INTERPOLATION_LINEAR);
	*/
	...
	anim->rotation_track_insert_key(rot_id, 0, submotion.bind_pose_rotation);
	...
	for(position_key key : submotion.position_keys){
		...
		if(key.time > max_time) max_time = key.time;
	}
	
	for(rotation_key key : submotion.rotation_keys){
	...
	// TODO: SCALEROTATION
	for(rotation_key key : submotion.scale_rotation_keys){
		if(key.time > max_time) max_time = key.time;
	}

	// ***** experiment, doesn't seem to help
	/*anim->position_track_insert_key(pos_id, max_time+0.01, vec3d_to_godot(submotion.packed.pose_position, true));
	anim->rotation_track_insert_key(rot_id, max_time+0.01, submotion.bind_pose_rotation);
	anim->scale_track_insert_key(scale_id, max_time+0.01, vec3d_to_godot(submotion.packed.pose_scale));
	*/
...
static Ref<Animation> _load_xsm_animation(Ref<FileAccess> const& file){
	...
	std::vector<node_submotion> node_submotions;
	...
	while(!file->eof_reached()){
		...
		if(header.type == 0xC9) read_xsm_metadata(file, metadata, header.version); //in zulu_moving.xsm, this is v1
		else if(header.type == 0xC8) read_node_submotion(file,&node_submotions);
		else if(header.type == 0xCA) read_xsm_bone_animation(file, bone_anim, header.version);
		else {
			UtilityFunctions::print(
			vformat("Unsupported XSM chunk: file: %s, type = %x, length = %x, version = %d", file->get_path(), header.type, header.length, header.version)
			);
	...
	for(skeletal_submotion submotion : bone_anim.submotions){
		float submotion_len = add_submotion(anim, submotion);
		if(submotion_len > animation_length) animation_length = submotion_len;
	}

	for(node_submotion submotion : node_submotions){
		float submotion_len = add_submotion(anim, submotion);
		if(submotion_len > animation_length) animation_length = submotion_len;
	}

	//its as if the last animation frame was being messed up
	//increase the animation time to hold it for longer temporarily
	anim->set_length(animation_length + 2);
