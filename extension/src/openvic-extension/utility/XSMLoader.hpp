...
float add_submotion(Ref<Animation> anim, T submotion) {
	...
	if(anim->track_find_key(pos_id, 0) != -1) anim->track_remove_key_at_time(pos_id, 0);
	if(anim->track_find_key(rot_id, 0) != -1) anim->track_remove_key_at_time(rot_id, 0);
	if(anim->track_find_key(scale_id, 0) != -1)anim->track_remove_key_at_time(scale_id, 0);
