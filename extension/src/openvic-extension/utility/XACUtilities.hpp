...
namespace OpenVic {
	...
	static bool read_string(Ref<FileAccess> const& file, String& str, bool replace_chars = true, bool log = false) {
		...
		if(file->get_length() - file->get_position() < length) return false;

		str = file->get_buffer(length).get_string_from_ascii();
		if(log) UtilityFunctions::print(vformat("before %s", str));
		...
		if(log) UtilityFunctions::print(vformat("after %s", str));
		return true;
	}

	template<typename T>
	static bool read_struct(Ref<FileAccess> const& file, T& t) {
		if(file->get_length() - file->get_position() < sizeof(T)) return false;
	...
	static bool read_struct_array(Ref<FileAccess> const& file, std::vector<T>& t, uint32_t size) {
		if(file->get_length() - file->get_position() < size*sizeof(T)) return false;
	...
	static Quaternion quat_v2_to_godot(quat_v2_t const& quat_in) {
		...
		if(!quat_out.is_normalized()){
			//Logger::error("quat isn't normalized");
			quat_out = Quaternion();
		}
	...
	static Color color_32_to_godot(color_32_t color_in){
	...
	static Color color_128_to_godot(color_128_t color_in){
	...
	static Color vec4d_to_godot_color(vec4d_t color_in){
		Color color_out = {
			color_in.x,
			color_in.y,
			color_in.z,
