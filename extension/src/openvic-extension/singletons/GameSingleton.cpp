...
Error GameSingleton::setup_game(int32_t bookmark_index) {
	...
	Bookmark const* bookmark = bookmark_manager.get_bookmark_by_index(bookmark_index);
	...
	CrimeManager const& crime_manager = get_definition_manager().get_crime_manager();
	...
	for (ProvinceInstance& province : instance_manager->get_map_instance().get_province_instances()) {
		province.set_crime(
			crime_manager.get_crime_modifier_by_index((province.get_index() - 1) % crime_manager.get_crime_modifier_count())
		);
	}
	...
	TechnologyManager const& technology_manager = get_definition_manager().get_research_manager().get_technology_manager();
	// TODO - remove this test starting research
	for (Technology const& technology : technology_manager.get_technologies()) {
