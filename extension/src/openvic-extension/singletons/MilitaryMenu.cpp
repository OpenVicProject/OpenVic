...
Dictionary MenuSingleton::make_leader_dict(LeaderInstance const& leader) {
...

	{
		...
		default:
			UtilityFunctions::push_error(
			    "Invalid branch type \"",
			    static_cast<int64_t>(leader.branch),
			    "\" for leader \"",
...

	{
		...
		modifier_value = definition_manager.get_military_manager().get_leader_trait_manager().get_leader_prestige_modifier() *
		                 prestige;
...
Dictionary MenuSingleton::make_unit_group_dict(UnitInstanceGroupBranched<Branch> const& unit_group) {
	...
	if (unit_group.get_leader() != nullptr) {
		...
		unit_group_dict[military_info_unit_group_leader_picture_key] = leader_dict.get(
		    military_info_leader_picture_key, Ref<Texture2D> {}
		);
		unit_group_dict[military_info_unit_group_leader_tooltip_key] = leader_dict.get(
		    military_info_leader_tooltip_key, String {}
		);
	}
	...
	if constexpr (Branch == LAND) {
		...
		unit_group_dict[military_info_unit_group_max_men_count_key] = _scale_land_unit_strength(
		    unit_group.get_total_max_strength()
		);
...
Dictionary MenuSingleton::get_military_menu_info() {
...

	{
		...
		if (research_contribution != fixed_point_t::_0) {
			...
			mobilisation_economy_impact_tooltip =
			    tr(research_contribution < fixed_point_t::_0
	...
	if (create_leader_count > 0) {
		...
		ret[military_info_create_leader_cost_key] = static_cast<real_t>(
		    definition_manager.get_define_manager().get_military_defines().get_leader_recruit_cost()
		);
	}
