...
Dictionary MenuSingleton::make_leader_dict(LeaderInstance const& leader) {
	...
	static const StringName leader_id_key = "leader_id";
	static const StringName leader_branch_key = "leader_branch";
	static const StringName leader_name_key = "leader_name";
	static const StringName leader_picture_key = "leader_picture";
	static const StringName leader_prestige_key = "leader_prestige";
	static const StringName leader_prestige_tooltip_key = "leader_prestige_tooltip";
	static const StringName leader_background_key = "leader_background";
	static const StringName leader_personality_key = "leader_personality";
	static const StringName leader_can_be_used_key = "leader_can_be_used";
	static const StringName leader_assignment_key = "leader_assignment";
	static const StringName leader_location_key = "leader_location";
	static const StringName leader_tooltip_key = "leader_tooltip";
	...
	// Picture
	leader_dict[leader_picture_key] = _get_leader_picture(leader);

	{
		// Generic data
		leader_dict[leader_id_key] = leader.get_unique_id();
		leader_dict[leader_branch_key] = static_cast<int32_t>(leader.get_branch());

		leader_dict[leader_can_be_used_key] = leader.get_can_be_used();

		UnitInstanceGroup const* group = leader.get_unit_instance_group();
		if (group != nullptr) {
			leader_dict[leader_assignment_key] = Utilities::std_to_godot_string(group->get_name());

			ProvinceInstance const* location = group->get_position();
			if (location != nullptr) {
				leader_dict[leader_location_key] = Utilities::std_to_godot_string(location->get_identifier());
			}
		}
	}

	{
		...
		leader_dict[leader_name_key] = std::move(leader_name);
	}

	{
		...
		static const String value_replace = "$VAL$";

		String prestige_tooltip =
			tr(prestige_localisation_key).replace(value_replace, Utilities::fixed_point_to_string_dp(prestige * 100, 2) + "%");
		...
		leader_dict[leader_prestige_key] = prestige.to_float();
		leader_dict[leader_prestige_tooltip_key] = std::move(prestige_tooltip);
	}

	static const String name_replace = "$NAME$";
	{
		// Background
		String background;
		...
		tooltip += "\n" + tr(background_localisation_key).replace(name_replace, background);

		leader_dict[leader_background_key] = std::move(background);
	}

	{
		...
		tooltip += "\n" + tr(personality_localisation_key).replace(name_replace, personality);

		leader_dict[leader_personality_key] = std::move(personality);
	}

	tooltip += _make_modifier_effects_tooltip(modifier_value);

	leader_dict[leader_tooltip_key] = std::move(tooltip);
...
Dictionary MenuSingleton::make_unit_group_dict(UnitInstanceGroupBranched<Branch> const& unit_group) {
	static const StringName unit_group_leader_picture_key = "unit_group_leader_picture";
	static const StringName unit_group_leader_tooltip_key = "unit_group_leader_tooltip";
	static const StringName unit_group_name_key = "unit_group_name";
	static const StringName unit_group_location_key = "unit_group_location";
	static const StringName unit_group_unit_count_key = "unit_group_unit_count";
	static const StringName unit_group_men_count_key = "unit_group_men_count"; // armies only
	static const StringName unit_group_max_men_count_key = "unit_group_max_men_count"; // armies only
	static const StringName unit_group_organisation_key = "unit_group_organisation";
	static const StringName unit_group_strength_key = "unit_group_strength";
	static const StringName unit_group_moving_tooltip_key = "unit_group_moving_tooltip";
	static const StringName unit_group_dig_in_tooltip_key = "unit_group_dig_in_tooltip"; // armies only
	static const StringName unit_group_combat_key = "unit_group_combat";
	...
	if (unit_group.get_leader() != nullptr) {
		...
		unit_group_dict[unit_group_leader_picture_key] = leader_dict.get(leader_picture_key, Ref<Texture2D> {});
		unit_group_dict[unit_group_leader_tooltip_key] = leader_dict.get(leader_tooltip_key, String {});
	}

	unit_group_dict[unit_group_name_key] = Utilities::std_to_godot_string(unit_group.get_name());
	if (unit_group.get_position() != nullptr) {
		unit_group_dict[unit_group_location_key] = Utilities::std_to_godot_string(unit_group.get_position()->get_identifier());
	}
	unit_group_dict[unit_group_unit_count_key] = static_cast<uint64_t>(unit_group.get_unit_count());

	unit_group_dict[unit_group_organisation_key] = unit_group.get_organisation_proportion().to_float();
	unit_group_dict[unit_group_strength_key] = unit_group.get_strength_proportion().to_float();
	...
	if (unit_group.is_moving()) {
		...
		unit_group_dict[unit_group_moving_tooltip_key] =
			tr(moving_localisation_key)
				.replace(
					moving_location_replace_key,
					tr(GUINode::format_province_name(
						destination != nullptr ? Utilities::std_to_godot_string(destination->get_identifier()) : String {},
						false
					))
				)
	...
	if constexpr (Branch == LAND) {
		unit_group_dict[unit_group_men_count_key] = _scale_land_unit_strength(unit_group.get_total_strength());
		unit_group_dict[unit_group_max_men_count_key] = _scale_land_unit_strength(unit_group.get_total_max_strength());
		...
		if (dig_in_level > 0) {
			...
			static const String days_replace = "$DAYS$";

			unit_group_dict[unit_group_dig_in_tooltip_key] =
				tr(dig_in_localisation_key).replace(days_replace, String::num_uint64(dig_in_level));
		}
	}

	if (unit_group.is_in_combat()) {
		unit_group_dict[unit_group_combat_key] = true;
	}
...
Dictionary MenuSingleton::make_in_progress_unit_dict() const {
	static const StringName unit_progress_key = "unit_progress";
	static const StringName unit_icon_key = "unit_icon";
	static const StringName unit_name_key = "unit_name";
	static const StringName unit_location_key = "unit_location";
	static const StringName unit_eta_key = "unit_eta";
	static const StringName unit_tooltip_key = "unit_tooltip";
	...
	in_progress_unit_dict[unit_progress_key] = progress.to_float();
	in_progress_unit_dict[unit_icon_key] = unit_type->get_icon();
	in_progress_unit_dict[unit_name_key] = Utilities::std_to_godot_string(unit_type->get_identifier());
	if (location != nullptr) {
		in_progress_unit_dict[unit_location_key] = Utilities::std_to_godot_string(location->get_identifier());
	}
	in_progress_unit_dict[unit_eta_key] = Utilities::date_to_string(eta);
	...
	if (!tooltip.is_empty()) {
		...
		in_progress_unit_dict[unit_tooltip_key] = tr(gathering_goods_localisation_key) + tooltip;
	}
...
Dictionary MenuSingleton::get_military_menu_info() {
	...
	// Military stats
	static const StringName war_exhaustion_key = "war_exhaustion";
	static const StringName war_exhaustion_tooltip_key = "war_exhaustion_tooltip";
	static const StringName supply_consumption_key = "supply_consumption";
	static const StringName supply_consumption_tooltip_key = "supply_consumption_tooltip";
	static const StringName organisation_regain_key = "organisation_regain";
	static const StringName organisation_regain_tooltip_key = "organisation_regain_tooltip";
	static const StringName land_organisation_key = "land_organisation";
	static const StringName land_organisation_tooltip_key = "land_organisation_tooltip";
	static const StringName naval_organisation_key = "naval_organisation";
	static const StringName naval_organisation_tooltip_key = "naval_organisation_tooltip";
	static const StringName land_unit_start_experience_key = "land_unit_start_experience";
	static const StringName naval_unit_start_experience_key = "naval_unit_start_experience";
	static const StringName unit_start_experience_tooltip_key = "unit_start_experience_tooltip";
	static const StringName recruit_time_key = "recruit_time";
	static const StringName recruit_time_tooltip_key = "recruit_time_tooltip";
	static const StringName combat_width_key = "combat_width";
	static const StringName combat_width_tooltip_key = "combat_width_tooltip";
	static const StringName dig_in_cap_key = "dig_in_cap";
	static const StringName military_tactics_key = "military_tactics";
	...
	ret[war_exhaustion_key] = vformat(war_exhaustion_template_string, war_exhaustion_string, max_war_exhaustion_string);
	...
	static const StringName value_replace = "$VALUE$";
	...
	ret[war_exhaustion_tooltip_key] = vformat(
		war_exhaustion_tooltip_template_string,
		tr(war_exhaution_localisation_key).replace(value_replace, war_exhaustion_string),
		_make_modifier_effect_contributions_tooltip(*country, *modifier_effect_cache.get_war_exhaustion_monthly()),
		tr(max_war_exhaution_localisation_key).replace(value_replace, max_war_exhaustion_string),
	...
	ret[supply_consumption_key] = country->get_supply_consumption().to_float();
	ret[supply_consumption_tooltip_key] = base_value_percent_tooltip +
	...
	ret[organisation_regain_key] = country->get_organisation_regain().to_float();
	{
		...
		if (morale != fixed_point_t::_0()) {
			organisation_regain_tooltip += from_technology_tooltip +
		...
		ret[organisation_regain_tooltip_key] = std::move(organisation_regain_tooltip);
	}

	ret[land_organisation_key] = country->get_land_organisation().to_float();
	ret[land_organisation_tooltip_key] = base_value_percent_tooltip +
		_make_modifier_effect_contributions_tooltip(*country, *modifier_effect_cache.get_land_organisation());

	ret[naval_organisation_key] = country->get_naval_organisation().to_float();
	ret[naval_organisation_tooltip_key] = base_value_percent_tooltip +
		_make_modifier_effect_contributions_tooltip(*country, *modifier_effect_cache.get_naval_organisation());

	ret[land_unit_start_experience_key] = country->get_land_unit_start_experience().to_float();
	ret[naval_unit_start_experience_key] = country->get_naval_unit_start_experience().to_float();
	{
		...
		ret[unit_start_experience_tooltip_key] = std::move(unit_start_experience_tooltip);
	}

	ret[recruit_time_key] = country->get_recruit_time().to_float();
	ret[recruit_time_tooltip_key] = base_value_percent_tooltip +
		_make_modifier_effect_contributions_tooltip(*country, *modifier_effect_cache.get_unit_recruitment_time());

	ret[combat_width_key] = country->get_combat_width();
	{
		static const StringName base_value_combat_width_localisation_key = "COMWID_BASE";
		static const String val_replace = "$VAL$";
		String combat_width_tooltip =
			tr(base_value_combat_width_localisation_key)
				.replace(
					val_replace,
					String::num_uint64(definition_manager.get_define_manager().get_military_defines().get_base_combat_width())
				);
		...
		if (combat_width != fixed_point_t::_0()) {
			combat_width_tooltip +=
				from_technology_tooltip + GUILabel::get_colour_marker() + "G" + String::num_int64(combat_width.to_int64_t());
		}
		ret[combat_width_tooltip_key] = std::move(combat_width_tooltip);
	}

	ret[dig_in_cap_key] = country->get_dig_in_cap();
	ret[military_tactics_key] = country->get_military_tactics().to_float();

	// Mobilisation
	static const StringName is_mobilised_key = "is_mobilised";
	static const StringName mobilisation_progress_key = "mobilisation_progress";
	static const StringName mobilisation_size_key = "mobilisation_size";
	static const StringName mobilisation_size_tooltip_key = "mobilisation_size_tooltip";
	static const StringName mobilisation_impact_tooltip_key = "mobilisation_impact_tooltip";
	static const StringName mobilisation_economy_impact_key = "mobilisation_economy_impact";
	static const StringName mobilisation_economy_impact_tooltip_key = "mobilisation_economy_impact_tooltip";

	ret[is_mobilised_key] = country->is_mobilised();
	// TODO - get mobilisation progress from SIM
	// ret[mobilisation_progress_key] = country->get_mobilisation_progress().to_float();
	ret[mobilisation_size_key] = static_cast<uint64_t>(country->get_mobilisation_potential_regiment_count());

	{
		...
		String mobilisation_size_tooltip =
		...
		ret[mobilisation_size_tooltip_key] = std::move(mobilisation_size_tooltip);
	}
	...
	ret[mobilisation_economy_impact_key] = country->get_mobilisation_economy_impact().to_float();

	{
		...
		ret[mobilisation_economy_impact_tooltip_key] = mobilisation_economy_impact_tooltip;
	}

	// Leaders
	static const StringName general_count_key = "general_count";
	static const StringName admiral_count_key = "admiral_count";
	static const StringName create_leader_count_key = "create_leader_count";
	static const StringName create_leader_cost_key = "create_leader_cost";
	static const StringName auto_create_leaders_key = "auto_create_leaders";
	static const StringName auto_assign_leaders_key = "auto_assign_leaders";
	static const StringName leaders_list_key = "leaders_list";

	ret[general_count_key] = static_cast<uint64_t>(country->get_general_count());
	ret[admiral_count_key] = static_cast<uint64_t>(country->get_admiral_count());
	const uint64_t create_leader_count = country->get_create_leader_count();
	if (create_leader_count > 0) {
		ret[create_leader_count_key] = static_cast<uint64_t>(country->get_create_leader_count());
	} else {
		ret[create_leader_cost_key] =
			definition_manager.get_define_manager().get_military_defines().get_leader_recruit_cost().to_float();
	}
	ret[auto_create_leaders_key] = country->get_auto_create_leaders();
	ret[auto_assign_leaders_key] = country->get_auto_assign_leaders();
	...
	if (country->has_leaders()) {
		...
		if (leaders.resize(leader_count) == OK) {
			...
			ret[leaders_list_key] = std::move(leaders);
		} else {
			UtilityFunctions::push_error(
	...
	// Armies and Navies
	static const StringName is_disarmed_key = "is_disarmed";
	static const StringName armies_key = "armies";
	static const StringName in_progress_brigades_key = "in_progress_brigades";
	static const StringName navies_key = "navies";
	static const StringName in_progress_ships_key = "in_progress_ships";

	ret[is_disarmed_key] = country->is_disarmed();
	...
	if (country->has_armies()) {
		...
		if (armies.resize(army_count) == OK) {
			...
			ret[armies_key] = std::move(armies);
		} else {
			UtilityFunctions::push_error(
...

	{
		...
		ret[in_progress_brigades_key] = in_progress_brigades;
	}
	...
	if (country->has_navies()) {
		...
		if (navies.resize(navy_count) == OK) {
			...
			ret[navies_key] = std::move(navies);
		} else {
			UtilityFunctions::push_error(
...

	{
		...
		ret[in_progress_ships_key] = in_progress_ships;
	}
