...
Dictionary MenuSingleton::make_leader_dict(LeaderInstance const& leader) {
...

	{
		...
		leader_dict[OV_INAME("leader_branch")] = static_cast<int32_t>(leader.branch);
		...
		if (group != nullptr) {
			leader_dict[OV_INAME("leader_assignment")] = convert_to<String>(group->get_name());
...
Dictionary MenuSingleton::make_in_progress_unit_dict() const {
	...
	in_progress_unit_dict[OV_INAME("unit_name")] = convert_to<String>(unit_type->get_identifier());
	if (location != nullptr) {
