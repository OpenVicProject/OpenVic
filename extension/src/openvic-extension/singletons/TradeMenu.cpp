		...
		for (GoodDefinition const* good_definition : good_category.get_good_definitions()) {
...

			{
				...
				good_dict[demand_tooltip_key] =
					...
					tr(supply_localisation_key).replace(val_replace, Utilities::fixed_point_to_string_dp(supply, 3)) + "\n" +
					tr(demand_localisation_key).replace(val_replace, Utilities::fixed_point_to_string_dp(demand, 3)) + "\n" +
					tr(actual_bought_localisation_key)
						.replace(
							val_replace, Utilities::fixed_point_to_string_dp(good_instance.get_quantity_traded_yesterday(), 3)
						);
			}
...
Dictionary MenuSingleton::get_trade_menu_trade_details_info( //
	...
	if (stockpile_cutoff_slider != nullptr) {
		...
		while (index < stockpile_cutoff_slider->get_max_value() &&
			   calculate_trade_menu_stockpile_cutoff_amount_fp(index * stockpile_cutoff_slider->get_step_size()) <
...
Dictionary MenuSingleton::get_trade_menu_tables_info() const {
	...
	for (GoodInstance const& good : good_instance_manager.get_good_instances()) {
		...
		for (size_t index = 0; index < 5; ++index) {
			...
			tooltip += vformat(
				top_producer_template_string, Utilities::std_to_godot_string(country->get_identifier()),
	...
	for (auto const& [good, good_data] : country->get_goods_data()) {
		...
		static const String money_replace = "$MONEY$";
		...
		good_trading_yesterday_tooltips.push_back(
			tooltip.replace(money_replace, Utilities::fixed_point_to_string_dp(stockpile_change_yesterday, 2))
				.replace(units_replace, Utilities::fixed_point_to_string_dp(stockpile_change_yesterday * good.get_price(), 2))
		);

		const float good_index = good.get_good_definition().get_index();

		if (good_data.government_needs != fixed_point_t::_0()) {
			government_needs.push_back({ good_index, good_data.government_needs.to_float() });
		}

		if (good_data.factory_demand != fixed_point_t::_0()) {
			factory_needs.push_back({ good_index, good_data.factory_demand.to_float() });
		}

		if (good_data.pop_demand != fixed_point_t::_0()) {
			pop_needs.push_back({ good_index, good_data.pop_demand.to_float() });
		}

		market_activity.push_back({ good_index, good_data.exported_amount.abs().to_float(),
									(good_data.exported_amount * good.get_price()).to_float() });

		stockpile.push_back({ good_index, good_data.stockpile_amount.to_float(),
