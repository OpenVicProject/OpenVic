...
bool SoundSingleton::load_title_theme() {
	...
	Dataloader::path_vector_t music_files = game_singleton->get_dataloader().lookup_files_in_dir_recursive(
	    music_directory, ".mp3"
	);
...
bool SoundSingleton::load_sounds() {
	...
	for (SoundEffect const& sound_inst : sound_manager.get_sound_effects()) {
		...
		if (stream.is_null()) {
			ERR_PRINT(
			    Utilities::format(
			        "Failed to load sound %s at path %s.",
			        convert_to<String>(sound_inst.get_identifier()),
