...
void CursorSingleton::_bind_methods() {
	...
	OV_BIND_METHOD(
		CursorSingleton::generate_resolution, { "cursor_name", "base_resolution_index", "target_resolution" }, "normal", 0,
		Vector2(64, 64)
	);
	OV_BIND_METHOD(CursorSingleton::get_cursor_names);

	ADD_PROPERTY(PropertyInfo(Variant::ARRAY, "cursor_names", PROPERTY_HINT_ARRAY_TYPE, "StringName"), "", "get_cursor_names");
}
...
PackedVector2Array CursorSingleton::get_hotspots(StringName const& name, int32_t res_index) const {
	...
	ERR_FAIL_INDEX_V_MSG(
		res_index, hotspots.size(), {}, vformat("Invalid hotspot index for cursor \"%s\": %d", name, res_index)
	);
...
Error CursorSingleton::load_cursors() {
	...
	Dataloader::path_vector_t cursor_files =
		game_singleton->get_dataloader().lookup_files_in_dir_recursive(cursor_directory, ".cur");
	...
	Dataloader::path_vector_t animated_cursor_files =
		game_singleton->get_dataloader().lookup_files_in_dir_recursive(cursor_directory, ".ani");
...
static constexpr int32_t _reverser_lookup[] { 0x0, 0x8, 0x4, 0xc, 0x2, 0xa, 0x6, 0xe, 0x1, 0x9, 0x5, 0xd, 0x3, 0xb, 0x7, 0xf };
...
_read_AND_mask(uint8_t const* data, int32_t pixel_x, int32_t pixel_y, int32_t x_dimension, int32_t offset) {
...
static void _pixel_palette_lookup(
	PackedByteArray const& data, PackedByteArray& pixel_data, uint32_t i, PackedByteArray const& palette, int32_t coord_x,
	int32_t coord_y, int32_t x_dimension, int32_t offset, bool transparent, int32_t bits_per_pixel
) {
	...
	// pixel bits serves as an index into the colour palette. We need to multiply the index by the number of bytes per colour
	// (4)
...
static void _read_24bit_pixel( //
	PackedByteArray const& image_data, PackedByteArray& pixel_data, int32_t i, int32_t offset, bool opaque
) {
	...
	for (int32_t i = 0; i < images_count; i++) {
		...
		if (image_data.slice(1, 4).get_string_from_ascii() == "PNG") {
			...
			if (bits_per_pixel <= 8 || bits_per_pixel == 24) {
				...
				Logger::error(
					"Invalid or Unsupported bits per pixel while loading cursor image, bpp: ", bits_per_pixel,
					"loading blank image instead"
				);
			}
			...
			for (int32_t row = 0; row < dimensions.y; row++) {
				for (int32_t col = 0; col < dimensions.x; col++) {
					...
					bool transparent = _read_AND_mask(image_data.ptr(), coord.x, coord.y, dimensions.x, mask_offset);
					if (bits_per_pixel <= 8) {
						// mostly legacy files, these ones all use a lookup into the colour palette
						_pixel_palette_lookup(
							image_data, pixel_data, i, palette, coord.x, coord.y, dimensions.x, offset, transparent,
							bits_per_pixel
						);
					} /*
						...
						_read_24bit_pixel(image_data, pixel_data, i, offset, transparent);
					} else if (bits_per_pixel == 32) {
						// What vic actually uses
						_read_32bit_pixel(image_data, pixel_data, i, offset, transparent);
					}
					i++;
				}
			}
...
bool CursorSingleton::_load_cursor_ani(StringName const& name, String const& path) {
	...
	ERR_FAIL_COND_V_MSG(err != OK || file.is_null(), false, vformat("Failed to open ani file: \"%s\"", path));
	...
	while (file->get_position() < riff_size) {
		...
		if (id == "LIST") {
			...
			if (size == 36) {
				int32_t header_size = file->get_32();
			}
			...
			if (resolutions.is_empty()) {
				...
				if (pair.images.size() != frames_by_resolution.size()) {
					Logger::error(
						"Malformatted .ani cursor file ", Utilities::godot_to_std_string(name),
...
bool CursorSingleton::_load_cursor_cur(StringName const& name, String const& path) {
	const Ref<FileAccess> file = FileAccess::open(path, FileAccess::ModeFlags::READ);
	const Error err = FileAccess::get_open_error();
	ERR_FAIL_COND_V_MSG(err != OK || file.is_null(), false, vformat("Failed to open cur file: \"%s\"", path));
	...
	cursors.emplace(
		name, cursor_asset_t { std::move(hotspots_by_resolution), std::move(frames_by_resolution), resolutions, 1 }
	);
	cursor_names.append(name);
