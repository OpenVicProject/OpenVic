...
TypedArray<ImageTexture> CursorSingleton::get_frames(StringName const& name, int32_t res_index) const {
	...
	ERR_FAIL_INDEX_V_MSG(
		res_index, images.size(), {}, Utilities::format("Invalid image index for cursor \"%s\": %d", name, res_index)
	);
...
PackedVector2Array CursorSingleton::get_hotspots(StringName const& name, int32_t res_index) const {
	...
	ERR_FAIL_INDEX_V_MSG(
		res_index, hotspots.size(), {}, Utilities::format("Invalid hotspot index for cursor \"%s\": %d", name, res_index)
	);
...
void CursorSingleton::generate_resolution(StringName const& name, int32_t base_res_index, Vector2 target_res) {
	...
	ERR_FAIL_INDEX_MSG(
		base_res_index, cursor.images.size(),
		Utilities::format("Invalid image index for cursor \"%s\": %d", name, base_res_index)
	);
...
static void
_read_32bit_pixel(PackedByteArray const& image_data, PackedByteArray& pixel_data, int32_t i, int32_t offset, bool opaque) {
	ERR_FAIL_COND_MSG(
		...
		if (id == "LIST") {
			...
			if (resolutions.is_empty()) {
				...
				if (pair.images.size() != frames_by_resolution.size()) {
					spdlog::error_s("Malformatted .ani cursor file {} had inconsistent number of images per cursor", name);
				}
				for (int32_t i = 0; i < pair.images.size(); i++) {
