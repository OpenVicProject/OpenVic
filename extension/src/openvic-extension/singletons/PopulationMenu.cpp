	...
	struct entry_visitor_t {
		...
		bool operator()(population_menu_t::country_entry_t const& country_entry) {
			if (start_counter-- <= 0) {
				...
				country_dict[OV_SNAME(size)] = type_safe::get(country_entry.country.get_total_population());
		...
		bool operator()(population_menu_t::state_entry_t const& state_entry) {
			...
			if (start_counter-- <= 0) {
				...
				state_dict[OV_SNAME(size)] = type_safe::get(state_entry.state.get_total_population());
		...
		bool operator()(population_menu_t::province_entry_t const& province_entry) {
			if (is_expanded && start_counter-- <= 0) {
				...
				province_dict[OV_SNAME(name)] = convert_to<String>(province_entry.province.get_identifier());
				province_dict[OV_SNAME(size)] = type_safe::get(province_entry.province.get_total_population());
...
TypedArray<Dictionary> MenuSingleton::get_population_menu_pop_rows(int32_t start, int32_t count) const {
	...
	for (int32_t idx = 0; idx < count; ++idx) {
		...
		pop_dict[OV_SNAME(size)] = type_safe::get(pop->get_size());
		pop_dict[OV_INAME("pop_type_icon")] = pop->get_type()->sprite;
		pop_dict[OV_SNAME(culture)] = convert_to<String>(pop->culture.get_identifier());
		pop_dict[OV_INAME("religion_icon")] = pop->religion.icon;
		if (pop->get_location() != nullptr) {
			pop_dict[OV_INAME("location")] = convert_to<String>(pop->get_location()->get_identifier());
		}
		...
		if (pop->get_rebel_type() != nullptr) {
			pop_dict[OV_INAME("rebel_icon")] = pop->get_rebel_type()->icon;
		}
		pop_dict[OV_INAME("size_change")] = type_safe::get(pop->get_total_change());
...
TypedArray<Dictionary> MenuSingleton::get_population_menu_pop_filter_info() const {
	...
	for (int32_t idx = 0; idx < array.size(); ++idx) {
		...
		filter_dict[OV_SNAME(change)] = type_safe::get(filter.promotion_demotion_change);
