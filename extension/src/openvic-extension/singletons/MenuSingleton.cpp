...
String MenuSingleton::_make_modifier_effect_contributions_tooltip(
	...
	modifier_sum.for_each_contributing_modifier(
	    effect,
	    [this, &effect, effect_value, &prefix, &suffix, &result](modifier_entry_t const& modifier_entry, fixed_point_t value)
	        -> void {
...
String MenuSingleton::_make_mobilisation_impact_tooltip() const {
	...
	PartyPolicy const* war_policy_issue =
	    war_policy_issue_group == nullptr
	        ? nullptr
...
void MenuSingleton::_bind_methods() {
	...
	ADD_SIGNAL(MethodInfo(
	    _signal_update_tooltip(),
	    PropertyInfo(Variant::STRING, "text"),
	    PropertyInfo(Variant::DICTIONARY, "substitution_dict"),
...
static TypedArray<Dictionary> _make_buildings_dict_array(ProvinceInstance const* province) {
	...
	if (buildings_array.resize(buildings.size()) == OK) {
		...
		UtilityFunctions::push_error(
		    ...
		    static_cast<int64_t>(buildings.size()),
		    ") for province ",
...
Dictionary MenuSingleton::get_province_info_from_number(int32_t province_number) const {
	...
	if (terrain_type != nullptr) {
		...
		static const String terrain_type_template_string =
		    "%s" + get_tooltip_separator() + "%s" + GUILabel::get_colour_marker() + "Y%s" + GUILabel::get_colour_marker() +
		    "!%s";
	...
	if (rgo.is_valid()) {
		...
		if (max_employee_count == 0) {
			...
			ret[province_info_rgo_employment_percentage_key] = static_cast<real_t>(
			    rgo.get_total_employees_count_cache() * fixed_point_t::_100 / max_employee_count
			);
		}
		...
		if (owner_job.has_value()) {
			...
			PopType const& owner_pop_type = GameSingleton::get_singleton()->get_pop_type(owner_pop_type_index);
			State const* state = province->get_state();
			...
			if (unlikely(state == nullptr)) {
				...
				const fixed_point_t effect_value =
				    state->get_total_population() == 0
				        ? fixed_point_t::_0
				        : fp::mul_div(
		...
		for (pop_type_index_t pop_type_index {}; pop_type_index < rgo.get_employee_count_per_type_cache().size();
			...
			for (Job const& job : production_type.get_jobs()) {
				...
				const fixed_point_t effect_value =
				    effect_multiplier == fixed_point_t::_1
...

		{
			...
			output_modifiers += _make_modifier_effect_contributions_tooltip(
			    *province,
			    *modifier_effect_cache.get_rgo_output_country(),
			    &output_multiplier,
			    modifier_effect_contributions_prefix
			);
			output_modifiers += _make_modifier_effect_contributions_tooltip(
			    *province,
			    *modifier_effect_cache.get_local_rgo_output(),
			    &output_multiplier,
			    modifier_effect_contributions_prefix
			);
			throughput_modifiers += _make_modifier_effect_contributions_tooltip(
			    *province,
			    *modifier_effect_cache.get_rgo_throughput_country(),
			    &throughput_multiplier,
			    modifier_effect_contributions_prefix
			);
			throughput_modifiers += _make_modifier_effect_contributions_tooltip(
			    *province,
			    *modifier_effect_cache.get_local_rgo_throughput(),
			    &throughput_multiplier,
			    modifier_effect_contributions_prefix
			);
			...
			if (production_type.get_is_farm_for_non_tech()) {
				output_modifiers += _make_modifier_effect_contributions_tooltip(
				    *province,
				    *modifier_effect_cache.get_farm_rgo_output_global(),
				    &output_multiplier,
				    modifier_effect_contributions_prefix
				);
				output_modifiers += _make_modifier_effect_contributions_tooltip(
				    *province,
				    *modifier_effect_cache.get_farm_rgo_output_local(),
				    &output_multiplier,
				    modifier_effect_contributions_prefix
				);
			...
			if (production_type.get_is_mine_for_non_tech()) {
				output_modifiers += _make_modifier_effect_contributions_tooltip(
				    *province,
				    *modifier_effect_cache.get_mine_rgo_output_global(),
				    &output_multiplier,
				    modifier_effect_contributions_prefix
				);
				output_modifiers += _make_modifier_effect_contributions_tooltip(
				    *province,
				    *modifier_effect_cache.get_mine_rgo_output_local(),
				    &output_multiplier,
				    modifier_effect_contributions_prefix
				);
			...
			ModifierEffectCache::good_effects_t const& good_effects = modifier_effect_cache.get_good_effects(
			...
			if (production_type.get_is_farm_for_tech()) {
				const fixed_point_t value = province->get_modifier_effect_value(
		...
		static const String rgo_production_template_string =
		    "%s\n%s%s" + get_tooltip_separator() + "%s%s%s\n%s " + GUILabel::get_colour_marker() + "G100%%" +
	...
	GFXPieChartTexture::godot_pie_chart_data_t pop_types =
	    GFXPieChartTexture::distribution_to_slices_array<const PopType, pop_sum_t>(
	...
	GFXPieChartTexture::godot_pie_chart_data_t ideologies =
	    GFXPieChartTexture::distribution_to_slices_array<const Ideology, fixed_point_t>(
	...
	if (!cores.empty()) {
		...
		if (cores_array.resize(cores.size()) == OK) {
			...
			UtilityFunctions::push_error(
			    ...
			    static_cast<int64_t>(cores.size()),
			    ") for province ",
			    convert_to<String>(province->get_identifier())
			);
		}
...
int32_t MenuSingleton::get_province_building_count() const {
	...
	return type_safe::get(manager.get_province_building_types().size());
}
...
PackedStringArray MenuSingleton::get_search_result_rows(int32_t start, int32_t count) const {
	...
	ERR_FAIL_INDEX_V_MSG(
	    start,
	    search_panel.result_indices.size(),
	    {},
	...
	if (start + count > search_panel.result_indices.size()) {
		UtilityFunctions::push_warning(
		    "Requested search result rows beyond the end of the result indices (",
		    start,
		    " + ",
		    count,
		    " > ",
		    static_cast<int64_t>(search_panel.result_indices.size()),
		    "), limiting to ",
		    static_cast<int64_t>(search_panel.result_indices.size() - start),
		    " rows."
		);
