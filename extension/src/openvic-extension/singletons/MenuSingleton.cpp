...
static TypedArray<Dictionary> _make_buildings_dict_array(
	...
	if (buildings_array.resize(buildings.size()) == OK) {
		for (size_t idx = 0; idx < buildings.size(); ++idx) {
			...
			building_dict[OV_SNAME(level)] = static_cast<int32_t>(type_safe::get(building.get_level()));
...
Dictionary MenuSingleton::get_province_info_from_number(int32_t province_number) const {
	...
	ret[OV_SNAME(province)] = convert_to<String>(province->get_identifier());
	...
	ret[OV_SNAME(life_rating)] = type_safe::get(province->get_life_rating());

	CountryInstance const* controller = province->get_controller();
	if (controller != nullptr) {
		ret[OV_SNAME(controller)] = convert_to<String>(controller->get_identifier());
	...
	if (rgo.is_valid()) {
		...
		ret[OV_INAME("rgo_icon")] = static_cast<uint64_t>(type_safe::get(rgo_good.index));
		...
		ret[OV_INAME("rgo_total_employees")] = type_safe::get(rgo.get_total_employees_count_cache());
		...
		ret[OV_INAME("rgo_employment_tooltip")] = Utilities::format(
			...
			tr(OV_INAME("PRODUCTION_FACTORY_EMPLOYEECOUNT_TOOLTIP2")).replace(
				OV_INAME("$EMPLOYEES$"), String::num_int64(type_safe::get(rgo.get_total_employees_count_cache()))
			).replace(
				OV_INAME("$EMPLOYEE_MAX$"), String::num_int64(type_safe::get(rgo.get_max_employee_count_cache()))
			),
	...
	if (crime != nullptr) {
		ret[OV_INAME("crime_name")] = convert_to<String>(crime->get_identifier());
		ret[OV_INAME("crime_icon")] = static_cast<int32_t>(crime->icon);
	}

	ret[OV_INAME("total_population")] = type_safe::get(province->get_total_population());
...
Dictionary MenuSingleton::get_topbar_info() const {
	...
	// Country / Ranking
	ret[OV_SNAME(country)] = convert_to<String>(country->get_identifier());
