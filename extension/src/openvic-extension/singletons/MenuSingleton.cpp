...
String MenuSingleton::_get_country_name(CountryInstance const& country) const {
	GovernmentType const* government_type = country.get_government_type();
	if (government_type != nullptr && !government_type->get_identifier().empty()) {
		const String government_name_key = Utilities::std_to_godot_string(
			StringUtils::append_string_views(country.get_identifier(), "_"sv, government_type->get_identifier())
		);
...
String MenuSingleton::_get_country_adjective(CountryInstance const& country) const {
	static constexpr std::string_view adjective = "_ADJ"sv;

	GovernmentType const* government_type = country.get_government_type();
	if (government_type != nullptr && !government_type->get_identifier().empty()) {
		const String government_adjective_key = Utilities::std_to_godot_string(
			StringUtils::append_string_views(country.get_identifier(), "_"sv, government_type->get_identifier(), adjective)
		);
...
String MenuSingleton::_make_modifier_effect_value_coloured(
	...
	const bool is_positive_green =
		(static_cast<uint8_t>(format_effect.get_format()) & //
		 (1 << ModifierEffect::FORMAT_POS_NEG_BIT_OFFSET)) == static_cast<uint8_t>(ModifierEffect::format_t::FORMAT_PART_POS);
...
String MenuSingleton::_make_mobilisation_impact_tooltip() const {
	...
	static const StringName mob_impact_limit_desc_key = "MOBILIZATION_IMPACT_LIMIT_DESC";
	static const String impact_replace = "$IMPACT$";
	static const String policy_replace = "$POLICY$";
	static const String units_replace = "$UNITS$";

	static const StringName mob_impact_limit_desc2_key = "MOBILIZATION_IMPACT_LIMIT_DESC2";
	static const String curr_replace = "$CURR$";
	static const String impact2_replace = "$IMPACT$";
	...
	return tr(mob_impact_limit_desc_key)
			   .replace(impact_replace, impact_string)
			   .replace(
				   policy_replace,
				   tr(war_policy_issue != nullptr
						  ? StringName { Utilities::std_to_godot_string(war_policy_issue->get_identifier()) }
						  : no_issue)
			   )
			   .replace(units_replace, String::num_uint64(country->get_mobilisation_max_regiment_count())) +
		"\n" +
		tr(mob_impact_limit_desc2_key)
			.replace(curr_replace, String::num_uint64(country->get_regiment_count()))
			.replace(impact2_replace, impact_string);
}
...
static TypedArray<Dictionary> _make_buildings_dict_array(ProvinceInstance const* province) {
	...
	static const StringName level_key = "level";
	static const StringName expansion_state_key = "expansion_state";
	static const StringName start_date_key = "start_date";
	static const StringName end_date_key = "end_date";
	static const StringName expansion_progress_key = "expansion_progress";
	...
	if (buildings_array.resize(buildings.size()) == OK) {
		for (size_t idx = 0; idx < buildings.size(); ++idx) {
			...
			building_dict[start_date_key] = Utilities::date_to_string(building.get_start_date());
			building_dict[end_date_key] = Utilities::date_to_string(building.get_end_date());
			building_dict[expansion_progress_key] = building.get_expansion_progress().to_float();
...
Dictionary MenuSingleton::get_province_info_from_index(int32_t index) const {
	...
	static const StringName province_key = "province";
	static const StringName state_key = "state";
	static const StringName slave_status_key = "slave_status";
	static const StringName colony_status_key = "colony_status";
	static const StringName terrain_type_key = "terrain_type";
	static const StringName terrain_type_tooltip_key = "terrain_type_tooltip";
	static const StringName life_rating_key = "life_rating";
	static const StringName controller_key = "controller";
	static const StringName controller_tooltip_key = "controller_tooltip";
	static const StringName rgo_icon_key = "rgo_icon";
	static const StringName rgo_production_tooltip_key = "rgo_production_tooltip";
	static const StringName rgo_total_employees_key = "rgo_total_employees";
	static const StringName rgo_employment_percentage_key = "rgo_employment_percentage";
	static const StringName rgo_employment_tooltip_key = "rgo_employment_tooltip";
	static const StringName rgo_output_quantity_yesterday_key = "rgo_output_quantity_yesterday";
	static const StringName rgo_revenue_yesterday_key = "rgo_revenue_yesterday";
	static const StringName crime_name_key = "crime_name";
	static const StringName crime_icon_key = "crime_icon";
	static const StringName total_population_key = "total_population";
	static const StringName pop_types_key = "pop_types";
	static const StringName pop_ideologies_key = "pop_ideologies";
	static const StringName pop_cultures_key = "pop_cultures";
	static const StringName cores_key = "cores";
	static const StringName buildings_key = "buildings";
	...
	ret[province_key] = Utilities::std_to_godot_string(province->get_identifier());

	State const* state = province->get_state();
	if (state != nullptr) {
		ret[state_key] = _get_state_name(*state);
	}

	ret[slave_status_key] = province->get_slave();

	ret[colony_status_key] = static_cast<int32_t>(province->get_colony_status());
	...
	if (terrain_type != nullptr) {
		...
		static const String terrain_replace = "$TERRAIN$";
		...
		ret[terrain_type_tooltip_key] = vformat(
			terrain_type_template_string, tr(terrain_type_localisation_key).replace(terrain_replace, tr(terrain_type_string)),
		...
		ret[terrain_type_key] = std::move(terrain_type_string);
	}
	...
	if (controller != nullptr) {
		ret[controller_key] = Utilities::std_to_godot_string(controller->get_identifier());

		static const StringName controller_localisation_key = "PV_CONTROLLER";
		static const String controller_template_string = "%s %s";
		ret[controller_tooltip_key] =
			vformat(controller_template_string, tr(controller_localisation_key), _get_country_name(*controller));
	}
	...
	if (rgo.is_valid()) {
		...
		ret[rgo_icon_key] = static_cast<int32_t>(rgo_good.get_index());

		ret[rgo_output_quantity_yesterday_key] = rgo.get_output_quantity_yesterday().to_float();
		ret[rgo_revenue_yesterday_key] = rgo.get_revenue_yesterday().to_float();
		ret[rgo_total_employees_key] = rgo.get_total_employees_count_cache();
		const pop_size_t max_employee_count = rgo.get_max_employee_count_cache();
		if (max_employee_count == 0) {
			ret[rgo_employment_percentage_key] = 100.0f;
		} else {
			ret[rgo_employment_percentage_key] =
		...
		if (owner_job.has_value()) {
			...
			if (unlikely(state == nullptr)) {
				Logger::error(
					"Province \"", province->get_identifier(),
					"\" has no state, preventing calculation of state-wide "
					"population proportion of RGO owner pop type \"",
					owner_pop_type.get_identifier(), "\""
				);
			} else {
		...
		for (auto const& [pop_type, employees_of_type] : rgo.get_employee_count_per_type_cache()) {
			...
			amount_of_employees_by_pop_type += vformat(
				amount_of_employees_by_pop_type_template_string, tr(Utilities::std_to_godot_string(pop_type.get_identifier())),
				employees_of_type
			);
		...
		static const String good_replace = "$GOODS$";
		static const String value_replace = "$VALUE$";
		...
		static const String base_replace = "$BASE$";
		...
		ret[rgo_production_tooltip_key] = vformat(
			rgo_production_template_string,
			tr(rgo_production_localisation_key)
				.replace(good_replace, tr(Utilities::std_to_godot_string(rgo_good.get_identifier())))
				.replace(value_replace, Utilities::fixed_point_to_string_dp(rgo.get_revenue_yesterday(), 3)),
			tr(max_output_localisation_key).replace(curr_replace, Utilities::fixed_point_to_string_dp(max_output, 2)),
			tr(output_explanation_localisation_key),
			tr(base_output_localisation_key).replace(base_replace, Utilities::fixed_point_to_string_dp(base_output, 2)),
		...
		static const String employees_replace = "$EMPLOYEES$";
		static const String employee_max_replace = "$EMPLOYEE_MAX$";
		...
		ret[rgo_employment_tooltip_key] = vformat(
			rgo_employment_template_string, tr(employment_localisation_key).replace(value_replace, {}),
			tr(employee_count_localisation_key)
				.replace(employees_replace, String::num_int64(rgo.get_total_employees_count_cache()))
				.replace(employee_max_replace, String::num_int64(rgo.get_max_employee_count_cache())),
			amount_of_employees_by_pop_type, tr(rgo_workforce_localisation_key), production_type.get_base_workforce_size(),
			size_string, tr(province_size_localisation_key),
	...
	if (crime != nullptr) {
		ret[crime_name_key] = Utilities::std_to_godot_string(crime->get_identifier());
		ret[crime_icon_key] = static_cast<int32_t>(crime->get_icon());
	}

	ret[total_population_key] = province->get_total_population();

	const auto make_pie_chart_tooltip = [this](
											HasGetIdentifierAndGetColour auto const* key, String const& identifier,
											float weight, float total_weight
										) -> String {
		static const String format_key = "%d%% %s";
		return vformat(format_key, static_cast<int32_t>(100.0f * weight / total_weight), tr(identifier));
	};
	...
	if (!pop_types.is_empty()) {
		ret[pop_types_key] = std::move(pop_types);
	}
	...
	if (!ideologies.is_empty()) {
		ret[pop_ideologies_key] = std::move(ideologies);
	}
	...
	if (!cultures.is_empty()) {
		ret[pop_cultures_key] = std::move(cultures);
	}
	...
	if (!cores.empty()) {
		...
		if (cores_array.resize(cores.size()) == OK) {
			...
			ret[cores_key] = std::move(cores_array);
		} else {
			UtilityFunctions::push_error(
	...
	if (!building_dict_array.is_empty()) {
		ret[buildings_key] = std::move(building_dict_array);
	}
...
String MenuSingleton::get_province_building_identifier(int32_t building_index) const {
	...
	std::vector<BuildingType const*> const& province_building_types = building_type_manager.get_province_building_types();
	ERR_FAIL_COND_V_MSG(
...
Dictionary MenuSingleton::get_topbar_info() const {
	...
	ret[industrial_rank_key] = static_cast<uint64_t>(country->get_industrial_rank());
	{
		...
		for (auto const& [state_name, power] : industrial_power_states) {
			...
			industrial_power_tooltip +=
				vformat(state_power_template_string, state_name, Utilities::fixed_point_to_string_dp(power, 3));
		}
		...
		for (auto const& [country_identifier, country_name, power] : industrial_power_from_investments) {
			static const String investment_power_template_string = "\n" + GUILabel::get_flag_marker() +
				"%s %s: " + GUILabel::get_colour_marker() + "Y%s" + GUILabel::get_colour_marker() + "!";

			industrial_power_tooltip += vformat(
				investment_power_template_string, country_identifier, country_name,
	...
	ret[military_rank_key] = static_cast<uint64_t>(country->get_military_rank());
	{
		...
		for (auto const& [source, power] :
			 { std::pair { military_power_from_land_key, country->get_military_power_from_land() },
	...
	// Technology
	static const String value_replace = "$VALUE$";
	static const String pop_type_replace = "$POPTYPE$";
	static const String fraction_replace = "$FRACTION$";
	static const String optimal_replace = "$OPTIMAL$";
	{
		...
		if (current_research != nullptr) {
			...
			static const String tech_replace = "$TECH$";
			static const String date_replace = "$DATE$";
			static const StringName research_invested_localisation_key = "TECHNOLOGYVIEW_RESEARCH_INVESTED_TOOLTIP";
			static const String invested_replace = "$INVESTED$";
			static const String cost_replace = "$COST$";
			...
			ret[research_tooltip_key] =
				tr(research_localisation_key)
					.replace(tech_replace, current_tech_localised)
					.replace(
						date_replace,
						Utilities::date_to_formatted_string(country->get_expected_research_completion_date(), false)
					) +
				"\n" +
				tr(research_invested_localisation_key)
					.replace(invested_replace, String::num_uint64(country->get_invested_research_points().to_int64_t()))
					.replace(cost_replace, String::num_uint64(country->get_current_research_cost().to_int64_t()));
		...
		for (auto const& [pop_type, research_points] : country->get_research_points_from_pop_types()) {
			...
			research_points_tooltip +=
				tr(pop_type_research_localisation_key)
					.replace(pop_type_replace, tr(Utilities::std_to_godot_string(pop_type->get_identifier())))
					.replace(value_replace, Utilities::fixed_point_to_string_dp(research_points, 2))
					.replace(
						fraction_replace,
					...
					.replace(
						optimal_replace,
						Utilities::fixed_point_to_string_dp(100 * pop_type->get_research_leadership_optimum(), 2)
					) +
				"\n";
		}
		...
		research_points_tooltip +=
			tr(daily_base_research_points_localisation_key)
				.replace(value_replace, Utilities::fixed_point_to_string_dp(daily_base_research_points, 2));
		...
		research_points_tooltip += "\n" +
			tr(daily_research_points_localisation_key)
				.replace(value_replace, Utilities::fixed_point_to_string_dp(country->get_daily_research_points(), 2));
		...
		research_points_tooltip +=
			tr(accumulated_research_points_localisation_key)
				.replace(val_replace, Utilities::fixed_point_to_string_dp(country->get_research_point_stockpile(), 1));
...

	{
		...
		for (auto const& [pop_type, leadership_points] : country->get_leadership_points_from_pop_types()) {
			...
			leadership_tooltip +=
				tr(pop_type_leadership_localisation_key)
					.replace(pop_type_replace, tr(Utilities::std_to_godot_string(pop_type->get_identifier())))
					.replace(value_replace, Utilities::fixed_point_to_string_dp(leadership_points, 2))
					.replace(
						fraction_replace,
						Utilities::fixed_point_to_string_dp(
							100 * country->get_pop_type_proportion(*pop_type) / country->get_total_population(), 2
		...
		leadership_tooltip +=
			tr(monthly_base_leadership_localisation_key)
				.replace(value_replace, Utilities::fixed_point_to_string_dp(monthly_base_leadership_points, 2));
		...
		leadership_tooltip += "\n" +
			tr(monthly_leadership_points_localisation_key)
				.replace(value_replace, Utilities::fixed_point_to_string_dp(country->get_monthly_leadership_points(), 2));
		...
		if (country->get_leadership_point_stockpile() >= max_leadership_point_stockpile) {
			...
			static const String max_replace = "$MAX$";
			leadership_tooltip +=
				tr(max_leadership_points_localisation_key)
					.trim_suffix("\n")
					.replace(max_replace, Utilities::fixed_point_to_string_dp(max_leadership_point_stockpile, 1));
		}
