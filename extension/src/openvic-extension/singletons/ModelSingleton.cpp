	...
	if (!attachments.empty()) {
		...
		if (attachments_array.resize(attachments.size()) == OK) {
			...
			UtilityFunctions::push_error(
			    "Failed to resize attachments array to the correct size (",
			    static_cast<int64_t>(attachments.size()),
			    ") for model for actor \"",
			    convert_to<String>(actor.get_name()),
			    "\""
			);
	...
	if constexpr (Branch == unit_branch_t::LAND) {
		...
		if (regiment_type->get_sprite_mount().empty() == regiment_type->get_sprite_mount_attach_node().empty()) {
			...
			UtilityFunctions::push_error(
			    ...
			    convert_to<String>(regiment_type->get_identifier()),
			    "\" has mount \"",
			    convert_to<String>(regiment_type->get_sprite_mount()),
			    "\" and attach node \"",
...
bool ModelSingleton::add_building_dict(
	...
	ERR_FAIL_NULL_V_MSG(
	    ...
	    Utilities::format(
	        ...
	        convert_to<String>(actor_name),
	        convert_to<String>(building.get_identifier()),
