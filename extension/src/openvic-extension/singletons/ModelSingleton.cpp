...
Dictionary ModelSingleton::get_animation_dict(GFX::Actor::Animation const& animation) {
	...
	dict[OV_SNAME(file)] = convert_to<String>(animation.get_file());
...
Dictionary ModelSingleton::get_model_dict(GFX::Actor const& actor) {
	...
	dict[OV_SNAME(file)] = convert_to<String>(actor.get_model_file());
	...
	if (!attachments.empty()) {
		...
		if (attachments_array.resize(attachments.size()) == OK) {
			...
			for (size_t idx = 0; idx < attachments_array.size(); ++idx) {
				...
				attachment_dict[OV_SNAME(node)] = convert_to<String>(attachment.get_attach_node());
...
bool ModelSingleton::add_unit_dict(
	...
	dict[OV_SNAME(culture)] = convert_to<String>(graphical_culture_type.get_identifier());
	...
	if (!mount_actor_name.empty() && !mount_attach_node_name.empty()) {
		...
		if (mount_actor != nullptr) {
			...
			dict[OV_INAME("mount_attach_node")] = convert_to<String>(mount_attach_node_name);
		} else {
	...
	dict[OV_INAME("flag_index")] = game_singleton->get_flag_sheet_index(country_definition.index, {});
	...
	dict[OV_SNAME(primary_colour)] = convert_to<Color>(country_definition.get_primary_unit_colour());
	dict[OV_SNAME(secondary_colour)] = convert_to<Color>(country_definition.get_secondary_unit_colour());
	dict[OV_SNAME(tertiary_colour)] = convert_to<Color>(country_definition.get_tertiary_unit_colour());
