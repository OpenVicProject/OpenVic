...
namespace OpenVic {
	...
	private:
		...
		struct cursor_asset_t {
			...
			int32_t animationLength; //1 for static cursors
	...
	public:
		...
		int32_t get_animationLength(godot::StringName const& name) const;
		godot::PackedVector2Array get_resolutions(godot::StringName const& name) const;
		godot::PackedFloat32Array get_displayRates(godot::StringName const& name) const;
