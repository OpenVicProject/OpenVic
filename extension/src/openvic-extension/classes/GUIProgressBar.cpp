...
Error GUIProgressBar::set_gfx_progress_bar(GFX::ProgressBar const* progress_bar) {
	...
	if (!progress_bar->get_progress_texture_file().empty()) {
		const StringName progress_texture_file = convert_to<String>(progress_bar->get_progress_texture_file());
	...
	set_custom_minimum_size(convert_to<Vector2>(progress_bar->get_size()));
