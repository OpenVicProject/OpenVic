...
void GUILabel::set_text_and_tooltip(
	...
	if (label_nullable != nullptr) {
		...
		label.set_text(label.tr(text_localisation_key));
		label.set_tooltip_string(label.tr(tooltip_localisation_key));
	}
}
...
void GUILabel::_bind_methods() {
	...
	ADD_PROPERTY(
	    PropertyInfo(Variant::BOOL, "auto_adjust_to_content_size"),
	    "set_auto_adjust_to_content_size",
	...
	ADD_PROPERTY(
	    PropertyInfo(Variant::OBJECT, "background", PROPERTY_HINT_RESOURCE_TYPE, "StyleBoxTexture"),
	    "set_background_stylebox",
	    "get_background"
	);
}
...
GUILabel::flag_segment_t GUILabel::make_flag_segment(String const& identifier) {
	...
	if (instance_manager != nullptr) {
		CountryInstance* country_instance = instance_manager->get_country_instance_manager().get_country_instance_by_identifier(
...
void GUILabel::separate_flag_segments(String const& string, Color const& colour, line_t& line) const {
	...
	while ((marker_pos = string.find(get_flag_marker(), start_pos)) != -1) {
		...
		if (start_pos >= string.length()) {
			UtilityFunctions::push_warning(
			    "Incomplete flag identifier \"",
			    string.substr(marker_pos),
			    "\" at end of line \"",
			    string,
