...
#include "gen/author_info.gen.hpp"
...
TypedDictionary<String, PackedStringArray> OVGame::get_author_info() {
	TypedDictionary<String, PackedStringArray> result;
	result["senior_developers"] = _generate_author_array(GAME_AUTHORS_SENIOR_DEVELOPERS);
	result["developers"] = _generate_author_array(GAME_AUTHORS_DEVELOPERS);
	result["contributors"] = _generate_author_array(GAME_AUTHORS_CONTRIBUTORS);
	result["consultants"] = _generate_author_array(GAME_AUTHORS_CONSULTANTS);
	result["artists"] = _generate_author_array(GAME_AUTHORS_ARTISTS);
	return result;
}

inline static Dictionary _generate_copyright_dictionary(GameComponentCopyright const& component) {
	Dictionary result;
	result["name"] = convert_to<String>(component.name);
	TypedArray<Dictionary> parts;
	parts.resize(component.parts.size());
	for (size_t index = 0; GameComponentCopyrightPart const& part : component.parts) {
...
TypedArray<Dictionary> OVGame::get_copyright_info() {
	TypedArray<Dictionary> result;
	result.resize(GAME_COPYRIGHT_INFO.size());
	for (size_t index = 0; GameComponentCopyright const& component : GAME_COPYRIGHT_INFO) {
...
TypedDictionary<String, String> OVGame::get_license_info() {
	TypedDictionary<String, String> result;
	for (GameLicense const& license : GAME_LICENSES) {
		result[convert_to<String>(license.license_name)] = convert_to<String>(license.license_body);
	}
	return result;
}

String OVGame::get_license_text() {
	return convert_to<String>(GAME_LICENSE_TEXT);
}

Dictionary OVGame::get_version_info() {
	Dictionary result;
	String tag = convert_to<String>(GAME_TAG);
	...
	result["release"] = convert_to<String>(GAME_RELEASE);
	result["hash"] = convert_to<String>(GAME_COMMIT_HASH);
	result["timestamp"] = GAME_COMMIT_TIMESTAMP;
