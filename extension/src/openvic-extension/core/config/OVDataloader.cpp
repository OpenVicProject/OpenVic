...
#include <openvic-dataloader/gen/author_info.gen.hpp>
#include <openvic-dataloader/gen/commit_info.gen.hpp>
#include <openvic-dataloader/gen/license_info.gen.hpp>
...
TypedDictionary<String, PackedStringArray> OVDataloader::get_author_info() {
	TypedDictionary<String, PackedStringArray> result;
	result["senior_developers"] = _generate_author_array(OVDL_AUTHORS_SENIOR_DEVELOPERS);
	result["contributors"] = _generate_author_array(OVDL_AUTHORS_CONTRIBUTORS);
	result["consultants"] = _generate_author_array(OVDL_AUTHORS_CONSULTANTS);
	return result;
}

inline static Dictionary _generate_copyright_dictionary(OvdlComponentCopyright const& component) {
	Dictionary result;
	result["name"] = convert_to<String>(component.name);
	TypedArray<Dictionary> parts;
	parts.resize(component.parts.size());
	for (size_t index = 0; OvdlComponentCopyrightPart const& part : component.parts) {
...
TypedArray<Dictionary> OVDataloader::get_copyright_info() {
	TypedArray<Dictionary> result;
	result.resize(OVDL_COPYRIGHT_INFO.size());
	for (size_t index = 0; OvdlComponentCopyright const& component : OVDL_COPYRIGHT_INFO) {
...
TypedDictionary<String, String> OVDataloader::get_license_info() {
	TypedDictionary<String, String> result;
	for (OvdlLicense const& license : OVDL_LICENSES) {
		result[convert_to<String>(license.license_name)] = convert_to<String>(license.license_body);
	}
	return result;
}

String OVDataloader::get_license_text() {
	return convert_to<String>(OVDL_LICENSE_TEXT);
}

Dictionary OVDataloader::get_version_info() {
	Dictionary result;
	String tag = convert_to<String>(OVDL_TAG);
	...
	result["release"] = convert_to<String>(OVDL_RELEASE);
	result["hash"] = convert_to<String>(OVDL_COMMIT_HASH);
	result["timestamp"] = OVDL_COMMIT_TIMESTAMP;
