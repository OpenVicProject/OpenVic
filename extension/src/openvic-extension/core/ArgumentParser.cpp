...
ArgumentOption::ArgumentOption(
    StringName const& name,
    Variant const& default_value,
    String const& description,
    PackedStringArray const& aliases,
    String const& option_args,
    BitField<ArgumentConflictType> conflict_type
) :
...
Ref<ArgumentOption> ArgumentOption::create(
    StringName const& p_name,
    Variant::Type p_type,
    String const& p_description,
    PackedStringArray const& p_aliases,
    String const& p_option_args,
    BitField<ArgumentConflictType> p_conflict_type
) {
...
ArgumentParser::ArgumentParser() {
	...
	options.push_back(
	    ArgumentOption::create(
	        "base-path",
	        Variant::STRING,
	        "Load Victoria 2 assets from a specific path.",
	        { "b" },
	        "[path]",
	...
	options.push_back(
	    ArgumentOption::create(
	        "search-path",
	        Variant::STRING,
	        "Search for Victoria 2 assets at a specific path.",
	        { "s" },
	        "[path]",
	        ArgumentOption::ARGUMENT_CONFLICT_SHORTHAND
	    )
...
Variant ArgumentParser::get_option_value(StringName const& p_arg_name) const {
	...
	if (it == arguments.end()) {
		...
		decltype(options)::Iterator it = const_cast<ArgumentParser*>(this)->_find_option(p_arg_name);
		ERR_FAIL_COND_V_MSG(
		    it == const_cast<ArgumentParser*>(this)->options.end(),
		    Variant(),
	...
	case VECTOR4I:    {
		...
		case VECTOR2I:
			ERR_FAIL_COND_V_EDMSG(
			    ...
			    Variant(),
			    vformat(
			        "'%s' must be a valid %s, '%s' is an invalid value.",
			        p_arg_name,
