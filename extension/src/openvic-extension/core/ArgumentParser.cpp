...
String ArgumentOption::get_help_string(bool p_is_rich) const {
	...
	if (type == Variant::STRING || type == Variant::STRING_NAME && default_value == Variant()) {
		help_type = vformat("Type: %s", Variant::get_type_name(get_type())).rpad(45);
	} else {
		help_type = vformat("Type: %s - Default Value: %s", Variant::get_type_name(type), default_value).rpad(45);
	}
...
String ArgumentParser::get_help(bool p_is_rich) const {
	...
	// TODO: fix license_info.gen.hpp failing build because GPLv3 license exceeds MSVC max string literal size
	// GAME_COPYRIGHT_INFO[0].parts.front().copyright_statements.front()
	String app_copyright = "(c) " + Utilities::std_to_godot_string("2023-present OpenVic Game contributors");
	if (p_is_rich) {
