...
1. Configure and build the extension in the project root:
    ```sh
    cmake --preset windows-x64-template_debug
    cmake --build --preset windows-x64-template_debug
    ```
    * Ensure the build produced an extension library for your platform in `game/bin/openvic`.
...
* To build for release use the `windows-x64-template_release` preset.

Each build copies the extension library into `game/bin/openvic` as a post-build step, so there is no separate install command. `cmake --install out/build/<preset> --config <configuration>` still works if you want to place the library somewhere else with `--prefix`.
...
Both produce the same artifact name and `game/bin/openvic` layout as the MSVC preset.
...
## Export
1. Build with either the debug or release preset as above.
